...
object Workers {
  def activityWorker(
      conf: MainConfig,
      connectionInfo: PhoenixConnectionInfo)(implicit ec: EC, ac: AS, mat: AM, cp: CP, sc: SC): Future[Unit] =
    Future {
      // Init
      val activityConnectors = Seq(
        AccountConnector,
        OrderConnector,
        GiftCardConnector,
      ...
      val kafkaInfo =
        KafkaConnectionInfo(broker = conf.kafkaBroker, schemaRegistryURL = conf.avroSchemaRegistryUrl)
      val activityProcessor = new ActivityProcessor(kafkaInfo, connectionInfo, activityConnectors)

      val avroProcessor =
        new AvroProcessor(schemaRegistryUrl = conf.avroSchemaRegistryUrl, processor = activityProcessor)
            ...
            Future {
              ...
              maybeTransformer match {
                case Some(transformer) ⇒
                  ...
                  val avroProcessor =
                    new AvroProcessor(schemaRegistryUrl = conf.avroSchemaRegistryUrl, processor = esProcessor)
  ...
  def scopedSearchViewWorkers(
    ...
    Future {
      ...
      val scopeProcessorFuture = Future {
        ...
        val avroProcessor =
          new AvroProcessor(schemaRegistryUrl = conf.avroSchemaRegistryUrl, processor = scopedProcessor)

        val consumer = new MultiTopicConsumer(
          topics = Seq(SCOPES_TABLE),
          broker = conf.kafkaBroker,
            ...
            Future {
              ...
              maybeTransformer match {
                case Some(transformer) ⇒
                  ...
                  // Start consuming & processing
                  Console.out.println(s"Scoped $topic Processor Reading from broker ${conf.kafkaBroker}")
                  consumer.readForever()
                case None ⇒
                  throw new IllegalArgumentException(s"The Topic '$topic' does not have a json transformer")
              }
  ...
  def topicTransformers(
    ...
    "geronimo_entities"                     → EntitiesAdminView(),
    "geronimo_entities"                     → EntitiesPublicView(),
    "geronimo_conten_type"                  → ContentTypesView()
  )
}
