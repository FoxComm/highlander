...
export default class Breadcrumbses extends Component {
  ...
  get crumbs(): Array<Element<*>> {
    return _.compact(_.map(this.props.routes, (route) => {
      ...
      } else if (_.isEmpty(route.indexRoute) && route.name === 'category-hero') {
        const categoryRoute = _.get(this.props, ['params', 'categoryName'], '');
        result = (
          <Crumb to="category-hero" params={this.props.params} name={humanize(categoryRoute, '+')} />
        );
