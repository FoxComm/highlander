...
object RmaLineItemUpdater {
  ...
  def addSkuLineItem(refNum: String, payload: RmaSkuLineItemsPayload, context: ObjectContext)(
    ...
    (for {
      ...
      // Inserts
      origin ← * <~ RmaLineItemSkus.create(
                  RmaLineItemSku(rmaId = rma.id, skuId = sku.id, skuShadowId = skuShadow.id))
  ...
  def deleteSkuLineItem(refNum: String, lineItemId: Int)(implicit ec: EC, db: DB): Result[Root] =
    (for {
      // Checks
      rma ← * <~ mustFindPendingRmaByRefNum(refNum)
      lineItem ← * <~ RmaLineItems
                  .join(RmaLineItemSkus)
                  .on(_.originId === _.id)
  ...
  def addGiftCardLineItem(refNum: String, payload: RmaGiftCardLineItemsPayload)(
    ...
    (for {
      ...
      // Inserts
      origin ← * <~ RmaLineItemGiftCards.create(
                  RmaLineItemGiftCard(rmaId = rma.id, giftCardId = oli._2.id))
  ...
  def addShippingCostItem(refNum: String, payload: RmaShippingCostLineItemsPayload)(
    ...
    (for {
      ...
      // Inserts
      origin ← * <~ RmaLineItemShippingCosts.create(
                  RmaLineItemShippingCost(rmaId = rma.id, shipmentId = shipment.id))
