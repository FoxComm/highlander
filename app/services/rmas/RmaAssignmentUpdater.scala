...
import services.{Failure, NotFoundFailure404, Result, RmaAssigneeNotFound}
...
object RmaAssignmentUpdater {
  ...
  def assign(refNum: String, requestedAssigneeIds: Seq[Int])
    (implicit db: Database, ec: ExecutionContext): Result[FullRmaWithWarnings] = (for {
    ...
    fullRma      ← * <~ fromRma(newRma).toXor
    warnings     = requestedAssigneeIds.diff(realAdminIds).map(NotFoundFailure404(StoreAdmin, _))
  } yield FullRmaWithWarnings(fullRma, warnings)).runTxn()
  ...
  def assignBulk(payload: payloads.RmaBulkAssigneesPayload)
    (implicit ec: ExecutionContext, db: Database, sortAndPage: SortAndPage): Result[BulkRmaUpdateResponse] = (for {
    ...
    _ ← RmaAssignments.createAll(for (r ← rmas; a ← admin) yield RmaAssignment(rmaId = r.id, assigneeId = a.id))
    ...
    result = allRmas,
    addFailures = adminNotFound ++ rmasNotFound)).transactionally.run()
