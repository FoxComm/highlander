...
object RmaLockUpdater {
  ...
  def getLockState(refNum: String)(implicit ec: EC, db: DB): Result[RmaLockResponse.Root] =
    (for {
  ...
  def lock(refNum: String, admin: StoreAdmin)(implicit ec: EC, db: DB): Result[RmaResponse.Root] =
    (for {
      rma  ← * <~ Rmas.mustFindByRefNum(refNum)
  ...
  def unlock(refNum: String)(implicit ec: EC, db: DB): Result[RmaResponse.Root] =
    (for {
      rma  ← * <~ Rmas.mustFindByRefNum(refNum)
      _    ← * <~ rma.mustBeLocked
