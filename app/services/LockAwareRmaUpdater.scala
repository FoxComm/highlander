...
object LockAwareRmaUpdater {
  ...
  def lock(refNum: String, admin: StoreAdmin)
    (implicit ec: ExecutionContext, db: Database): Result[RmaResponse.Root] = {
  ...
  def unlock(refNum: String)(implicit db: Database, ec: ExecutionContext): Result[RmaResponse.Root] = {
