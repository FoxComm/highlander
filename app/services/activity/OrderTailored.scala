...
object OrderTailored {
  ...
  case class OrderShippingMethodUpdated(order: FullOrder.Root,
                                        shippingMethod: Option[ShippingMethod],
                                        admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderShippingMethodUpdated]

  case class OrderShippingMethodRemoved(order: FullOrder.Root,
                                        shippingMethod: ShippingMethod,
                                        admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderShippingMethodRemoved]

  /* Order Shipping Addresses */
  case class OrderShippingAddressAdded(order: FullOrder.Root,
                                       shippingAddress: Addresses.Root,
                                       admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderShippingAddressAdded]

  case class OrderShippingAddressUpdated(order: FullOrder.Root,
                                         shippingAddress: Addresses.Root,
                                         admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderShippingAddressUpdated]

  case class OrderShippingAddressRemoved(order: FullOrder.Root,
                                         shippingAddress: Addresses.Root,
                                         admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderShippingAddressRemoved]

  /* Order Payment Methods */
  case class OrderPaymentMethodAddedCreditCard(order: FullOrder.Root,
                                               creditCard: CreditCardsResponse.Root,
                                               admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderPaymentMethodAddedCreditCard]

  case class OrderPaymentMethodAddedGiftCard(order: FullOrder.Root,
                                             giftCard: GiftCardResponse.Root,
                                             amount: Int,
                                             admin: Option[StoreAdminResponse.Root])
      extends ActivityBase[OrderPaymentMethodAddedGiftCard]

  case class OrderPaymentMethodUpdatedGiftCard(order: FullOrder.Root,
                                               giftCard: GiftCardResponse.Root,
