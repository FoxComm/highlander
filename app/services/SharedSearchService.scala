...
object SharedSearchService {
  def getAll(admin: StoreAdmin, rawScope: Option[String])(
      implicit ec: EC, db: DB): Result[Seq[SharedSearch]] =
    (for {
  ...
  def getAssociates(code: String)(implicit ec: EC, db: DB): Result[Seq[StoreAdminResponse.Root]] =
    (for {
  ...
  def create(admin: StoreAdmin, payload: SharedSearchPayload)(
      implicit ec: EC, db: DB): Result[SharedSearch] =
    (for {
  ...
  def delete(admin: StoreAdmin, code: String)(implicit ec: EC, db: DB): Result[Unit] =
    (for {
      search ← * <~ mustFindActiveByCode(code)
  ...
  def associate(admin: StoreAdmin, code: String, requestedAssigneeIds: Seq[Int])(
      implicit ec: EC, db: DB, ac: AC): Result[TheResponse[SharedSearch]] =
    (for {
  ...
  def unassociate(admin: StoreAdmin, code: String, assigneeId: Int)(
      implicit ec: EC, db: DB, ac: AC): Result[SharedSearch] =
    (for {
