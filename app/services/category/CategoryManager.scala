...
object CategoryManager {
  ...
  def getForm(id: Int)(implicit ec: EC, db: DB): Result[CategoryFormResponse.Root] = (for {
  ...
  def getShadow(formId: Int, contextName: String)
    (implicit ec: EC, db: DB): Result[CategoryShadowResponse.Root] = (for {
    context  ← * <~ contextByName(contextName)
    category ← * <~ Categories.withContextAndForm(context.id, formId).one.
  ...
  def getCategory(categoryId: Int, contextName: String)
    (implicit ec: EC, db: DB): Result[FullCategoryResponse.Root] = getCategoryFull(categoryId, contextName)
    .map(c ⇒ FullCategoryResponse.build(c.category, c.form, c.shadow)).run()

  def createCategory(admin: StoreAdmin, payload: CreateFullCategory, contextName: String)
    (implicit ec: EC, db: DB, ac: AC): Result[FullCategoryResponse.Root] = (for {
    ...
    categoryResponse = FullCategoryResponse.build(category, insertResult.form, insertResult.shadow)
    _                ← * <~ LogActivity.fullCategoryCreated(Some(admin), categoryResponse, ObjectContextResponse.build(context))
  } yield categoryResponse).runTxn()

  def updateCategory(admin: StoreAdmin, categoryId: Int, payload: UpdateFullCategory, contextName: String)
    (implicit ec: EC, db: DB, ac: AC): Result[FullCategoryResponse.Root] = (for {

    context          ← * <~ contextByName(contextName)
    category         ← * <~ categoryById(categoryId, context)
    updatedCategory  ← * <~ ObjectUtils.update(category.formId, category.shadowId, payload.form.attributes,
      payload.shadow.attributes)
    commit           ← * <~ ObjectUtils.commit(updatedCategory.form, updatedCategory.shadow, updatedCategory.updated)
    category         ← * <~ updateCategoryHead(category, updatedCategory.shadow, commit)
    categoryResponse = FullCategoryResponse.build(category, updatedCategory.form, updatedCategory.shadow)
  ...
  def getIlluminatedCategory(categoryId: Int, contextName: String)
    (implicit ec: EC, db: DB): Result[IlluminatedCategoryResponse.Root] = getCategoryFull(categoryId, contextName)
    .map(c ⇒ IlluminatedCategoryResponse.build(
      IlluminatedCategory.illuminate(c.context, c.category, c.form, c.shadow))).run()

  def getIlluminatedCategoryAtCommit(categoryId: Int, contextName: String, commitId: Int)
    (implicit ec: EC, db: DB): Result[IlluminatedCategoryResponse.Root] = (for {

    context        ← * <~ contextByName(contextName)
    category       ← * <~ categoryById(categoryId, context)
    commit         ← * <~ ObjectCommits.filter(commit ⇒ commit.id === commitId && commit.formId === categoryId).one.
  ...
  } yield IlluminatedCategoryResponse.build(
      IlluminatedCategory.illuminate(context, category, categoryForm, categoryShadow))
    ).run()

  private def updateCategoryHead(category: Category, categoryShadow: ObjectShadow,
    maybeCommit: Option[ObjectCommit])(implicit ec: EC): DbResultT[Category] =
    maybeCommit match {
      case Some(commit) ⇒
        DbResultT(Categories.update(category,category.copy(shadowId = categoryShadow.id, commitId = commit.id)))
      case None ⇒
  ...
  private def categoryById(categoryId: Int, context: ObjectContext)(implicit ec: EC): DbResult[Category] =
    Categories.withContextAndCategory(context.id, categoryId).one.
      mustFindOr(CategoryNotFoundForContext(categoryId, context.id))

  private def getCategoryFull(categoryId: Int, contextName: String)
    (implicit ec: EC, db: DB): DbResultT[CategoryFull] = for {
    context  ← * <~ contextByName(contextName)
