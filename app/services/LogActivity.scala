...
object LogActivity {
  ...
  def customerCreated(customer: CustomerResponse, admin: Option[StoreAdmin])
    (implicit ec: EC, ac: ActivityContext): DbResult[Activity] = admin match {
      case Some(a) ⇒
  ...
  def addressUpdated(originator: Originator, customer: Customer, newAddress: Address, newRegion: Region,
    oldAddress: Address, oldRegion: Region)(implicit ec: EC, ac: ActivityContext): DbResult[Activity] =
  ...
  def orderLineItemsUpdated(order: FullOrder.Root, oldQtys: Map[String, Int], payload: Seq[UpdateLineItemsPayload],
    admin: Option[StoreAdmin] = None)(implicit ec: EC, ac: ActivityContext): DbResult[Activity] =
  ...
  private def buildOriginator(originator: Originator)
    (implicit ec: EC, ac: ActivityContext): Option[AdminResponse] = originator match {
