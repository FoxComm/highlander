...
case class ShippingPriceRule(id: Int = 0,
                             name: String,
                             ruleType: ShippingPriceRule.RuleType,
                             flatPrice: Int,
                             flatMarkup: Int)
...
object ShippingPriceRule {
  ...
  implicit val RuleTypeColumn: JdbcType[RuleType] with BaseTypedType[RuleType] =
    MappedColumnType.base[RuleType, String]({
      case t ⇒ t.toString.toLowerCase
    }, {
      case "flat"        ⇒ Flat
      case "fromcarrier" ⇒ FromCarrier
      case unknown ⇒
        throw new IllegalArgumentException(s"cannot map price_type column to type $unknown")
    })
}

class ShippingPriceRules(tag: Tag)
    extends FoxTable[ShippingPriceRule](tag, "shipping_price_rules") {
  ...
  def * =
    (id, name, ruleType, flatPrice, flatMarkup) <> ((ShippingPriceRule.apply _).tupled, ShippingPriceRule.unapply)
}

object ShippingPriceRules
    extends FoxTableQuery[ShippingPriceRule, ShippingPriceRules](new ShippingPriceRules(_))
