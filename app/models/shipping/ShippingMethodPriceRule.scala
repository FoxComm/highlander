...
class ShippingMethodsPriceRules(tag: Tag)
    extends FoxTable[ShippingMethodPriceRule](tag, "shipping_methods_price_rules") {
  ...
  def * =
    (id, shippingMethodId, shippingPriceRuleId, ruleRank) <> ((ShippingMethodPriceRule.apply _).tupled, ShippingMethodPriceRule.unapply)
}

object ShippingMethodsPriceRules
    extends FoxTableQuery[ShippingMethodPriceRule, ShippingMethodsPriceRules](
