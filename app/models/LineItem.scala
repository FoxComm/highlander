...
object LineItems {
  ...
  def findByCart(cart: Cart)(implicit db: Database) = {db.run(_findByCartId(cart.id)) }

  def _findByCartId(cartId: Rep[Int]) = { lineItems.filter(_.parentId === cartId).filter(_.parentType === "cart").result }
