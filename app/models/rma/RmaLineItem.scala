...
case class RmaLineItem(id: Int = 0,
...
object RmaLineItem {
  ...
  def buildSku(rma: Rma,
               reason: RmaReason,
               origin: RmaLineItemSku,
               payload: RmaSkuLineItemsPayload): RmaLineItem = {
    RmaLineItem(
        rmaId = rma.id,
        reasonId = reason.id,
...
class RmaLineItems(tag: Tag) extends FoxTable[RmaLineItem](tag, "rma_line_items") {
  ...
  def * =
    (id,
     ...
     createdAt) <> ((RmaLineItem.apply _).tupled, RmaLineItem.unapply)

  def skuLineItems = foreignKey(RmaLineItemSkus.tableName, originId, RmaLineItemSkus)(_.id)
}

object RmaLineItems
    extends FoxTableQuery[RmaLineItem, RmaLineItems](new RmaLineItems(_))
