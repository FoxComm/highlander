...
case class RmaLineItemShippingCost(
    id: Int = 0, rmaId: Int, shipmentId: Int, createdAt: Instant = Instant.now)
    extends FoxModel[RmaLineItemShippingCost]

object RmaLineItemShippingCost {}

class RmaLineItemShippingCosts(tag: Tag)
    extends FoxTable[RmaLineItemShippingCost](tag, "rma_line_item_shipments") {
  ...
  def * =
    (id, rmaId, shipmentId, createdAt) <> ((RmaLineItemShippingCost.apply _).tupled, RmaLineItemShippingCost.unapply)

  def shipment = foreignKey(Shipments.tableName, shipmentId, Shipments)(_.id)
}

object RmaLineItemShippingCosts
    extends FoxTableQuery[RmaLineItemShippingCost, RmaLineItemShippingCosts](
  ...
  def findLineItemsByRma(
      rma: Rma): Query[(Shipments, RmaLineItems), (Shipment, RmaLineItem), Seq] =
    for {
