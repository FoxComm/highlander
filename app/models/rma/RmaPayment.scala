...
case class RmaPayment(id: Int = 0,
                      rmaId: Int = 0,
                      amount: Int = 0,
...
object RmaPayment {
  ...
  def build(method: PaymentMethod, rmaId: Int, amount: Int, currency: Currency): RmaPayment =
    method match {
      case gc: GiftCard ⇒
        RmaPayment(rmaId = rmaId,
                   amount = amount,
                   currency = currency,
...
class RmaPayments(tag: Tag) extends FoxTable[RmaPayment](tag, "rma_payments") {
  ...
  def * =
    (id, rmaId, amount, currency, paymentMethodId, paymentMethodType) <> ((RmaPayment.apply _).tupled,
        RmaPayment.unapply)
...
object RmaPayments
    extends FoxTableQuery[RmaPayment, RmaPayments](new RmaPayments(_))
  ...
  def findAllPaymentMethodmentsFor(
      rmaId: Int): Query[(RmaPayments, CreditCards), (RmaPayment, CreditCard), Seq] = {
    for {
