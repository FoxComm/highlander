...
case class RmaLineItemSku(
    id: Int = 0, rmaId: Int, skuId: Int, skuShadowId: Int, createdAt: Instant = Instant.now)
...
class RmaLineItemSkus(tag: Tag) extends FoxTable[RmaLineItemSku](tag, "rma_line_item_skus") {
  ...
  def * =
    (id, rmaId, skuId, skuShadowId, createdAt) <> ((RmaLineItemSku.apply _).tupled, RmaLineItemSku.unapply)
...
object RmaLineItemSkus
    extends FoxTableQuery[RmaLineItemSku, RmaLineItemSkus](new RmaLineItemSkus(_))
    with ReturningId[RmaLineItemSku, RmaLineItemSkus] {

  val returningLens: Lens[RmaLineItemSku, Int] = lens[RmaLineItemSku].id
  ...
  def findLineItemsByRma(rma: Rma): Query[(Skus, ObjectForms, ObjectShadows, RmaLineItems),
                                          (Sku, ObjectForm, ObjectShadow, RmaLineItem),
                                          Seq] =
    for {
