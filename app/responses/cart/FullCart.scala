...
import failures.CouponFailures.{CouponCodeNotFound, CouponWithCodeCannotBeFound}
...
import responses.{Addresses, CustomerResponse, GiftCardResponse, ResponseItem, ShippingMethods}
...
object FullCart {
  ...
  def fromCart(cart: Cart)(implicit db: DB, ec: EC): DbResultT[Root] = {
    ...
    for {
      ...
      giftCards  ← * <~ OrderLineItemGiftCards.findLineItemsByCordRef(cart.refNum).result
      shipMethod ← * <~ shipping.ShippingMethods.forCordRef(cart.refNum).one
      shipAddress ← * <~ Addresses
                     .forOrderRef(cart.refNum)
      ...
      payState     ← * <~ CartQueries.getPaymentState(cart.refNum)
      orderPromo   ← * <~ OrderPromotions.filterByOrderRef(cart.refNum).one
      promoDetails ← * <~ fetchPromoDetails(context, orderPromo)
      lineItemAdj  ← * <~ OrderLineItemAdjustments.findByOrderRef(cart.refNum).result
    } yield
      build(
          cart = cart,
          customer = customer,
  ...
  def build(cart: Cart,
    ...
    Root(referenceNumber = cart.referenceNumber,
  ...
  private def totals(cart: Cart): Totals =
  ...
  private def fetchPromoDetails(context: ObjectContext, cartPromo: Option[OrderPromotion])(
      implicit db: DB,
      ec: EC): DBIO[PromoDetails] = cartPromo match {
    case Some(op) ⇒
