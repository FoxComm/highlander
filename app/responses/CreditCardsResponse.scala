...
object CreditCardsResponse {
  final case class Root(id: Int, customerId: Int, holderName: String, lastFour: String, expMonth: Int, expYear: Int,
    ...
    inWallet: Boolean = true, address: responses.Addresses.Root) extends ResponseItem
  ...
  def build(cc: CreditCard, region: Region): Root =
    ...
    zipCheck = cc.zipCheck, inWallet = cc.inWallet, address = responses.Addresses.buildFromCreditCard(cc, region))
}
