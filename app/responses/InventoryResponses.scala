...
object InventoryResponses {
  ...
  case class SkuCounts(
      onHand: Int, onHold: Int, reserved: Int, safetyStock: Option[Int], afs: Int, afsCost: Int)

  object SkuCounts {

    def build[A <: InventorySummaryBase[A]](summary: A, price: Int): SkuCounts = summary match {
      case sellable: SellableInventorySummary ⇒
        SkuCounts(onHand = sellable.onHand,
                  onHold = sellable.onHold,
                  reserved = sellable.reserved,
                  safetyStock = Some(sellable.safetyStock),
                  afs = sellable.availableForSale,
                  afsCost = sellable.availableForSaleCost(price))
      case _ ⇒
        SkuCounts(onHand = summary.onHand,
                  onHold = summary.onHold,
                  reserved = summary.reserved,
                  safetyStock = None,
                  afs = summary.availableForSale,
                  afsCost = summary.availableForSaleCost(price))
    }
  }
