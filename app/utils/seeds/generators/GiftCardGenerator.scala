...
trait GiftCardGenerator {
  ...
  def generateGiftCardPurchase(customerId: Int)(implicit db: Database) : DbResultT[GiftCard] = for { 
    order ← * <~ Orders.create(Order(state = Order.ManualHold, customerId = customerId, referenceNumber = giftCardOrderReferenceNum))
