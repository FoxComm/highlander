...
trait CouponSeeds {
  ...
  def createCoupons(promotions: Seq[BasePromotion])(implicit db: Database): DbResultT[Seq[BaseCoupon]] = for {
  ...
  def insertCoupon(promo: BasePromotion, payload: CreateCoupon, context: ObjectContext)
    (implicit db: Database): DbResultT[BaseCoupon] = for {
    // Create coupon
    ...
    unsaved     = codes.map { c ⇒ CouponCode(couponFormId = ins.form.id, code = c) }
