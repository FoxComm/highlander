...
resource "google_compute_instance" "tiny-consul" {
    ...
    provisioner "remote-exec" {
        inline = [
          ...
          "sudo /opt/sensu/embedded/bin/update-client-cfg.rb ${var.datacenter}-consul-server ${google_compute_instance.tiny-consul.network_interface.0.address}",
...
resource "google_compute_instance" "tiny-frontend" {
    ...
    provisioner "remote-exec" {
        inline = [
          ...
          "sudo /opt/sensu/embedded/bin/update-client-cfg.rb ${var.datacenter}-frontend ${google_compute_instance.tiny-consul.network_interface.0.address}",
          "sudo systemctl restart sensu-client"
        ]
    }
}
...
resource "google_compute_instance" "tiny-backend" {
    ...
    provisioner "remote-exec" {
        inline = [
          "/usr/local/bin/bootstrap.sh",
          "/usr/local/bin/bootstrap_consul.sh ${var.datacenter} ${google_compute_instance.tiny-consul.network_interface.0.address}",
          "sudo /opt/sensu/embedded/bin/update-client-cfg.rb ${var.datacenter}-backend ${google_compute_instance.tiny-consul.network_interface.0.address}",
          "sudo systemctl restart sensu-client"
        ]
    }
}
