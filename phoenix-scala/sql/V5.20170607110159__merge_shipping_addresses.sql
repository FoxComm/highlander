create table address_cord(
  ...
  cord_ref text not null unique references cords(reference_number) on delete restrict,
  address_id int not null references addresses(id) on delete restrict,
  unique (cord_ref, address_id)
);
...
insert into address_cord (cord_ref, address_id)
  ...
  from order_shipping_addresses as osa
    ...
    inner join addresses as a on a.account_id = coalesce(c.account_id, o.account_id) and a.address1 = osa.address1
