create or replace function update_carts_view_from_shipping_addresses_fn() returns trigger as $$
...
begin
  ...
  update carts_search_view set
    ...
    from (select
        ...
        from carts as c
        left join order_shipping_addresses as osa on (c.reference_number = osa.cord_ref)
...
create trigger update_carts_view_from_order_shipping_addresses
    after update or insert on order_shipping_addresses
    for each row
    execute procedure update_carts_view_from_shipping_addresses_fn();

create trigger update_carts_view_from_regions
    after update or insert on regions
    for each row
    execute procedure update_carts_view_from_shipping_addresses_fn();

create trigger update_carts_view_from_countries
