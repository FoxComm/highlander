...
begin
  ...
  update products_catalog_view set archived_at = to_json_timestamp(now()::timestamp) where id IN (
    select p.id
    from products as p
...
update products_catalog_view set archived_at = to_json_timestamp(now()::timestamp) where id IN (select p.id
  from products as p
  inner join products_catalog_view as pv on (pv.id = p.id)
  inner join object_forms as f on (f.id = p.form_id)
  inner join object_shadows as s on (s.id = p.shadow_id)
  where
    ...
    from products as p
      ...
      inner join object_forms as f on (f.id = p.form_id)
      inner join object_shadows as s on (s.id = p.shadow_id)
      -- check that product not in products_catalog_view yet
      -- or archived
    where (pv.id is null or pv.archived_at is not null) and
    ...
    if array_length(insert_ids, 1) > 0 then
      ...
      on conflict(id, lower(context::text)) do update set archived_at = null;
      end if;
