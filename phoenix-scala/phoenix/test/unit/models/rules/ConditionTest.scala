    ...
    "#matches(Int, Condition)" - {
      ...
      "NotEquals" - {
        "returns false when values are equal" in {
          val c =
            Factories.condition.copy(operator = Condition.NotEquals, valInt = Some(50), valString = None)
          val result = Condition.matches(50, c)
          result must === (false)
        }
      ...
      "GreaterThan" - {
        "returns true when value is greater than condition" in {
          val c =
            Factories.condition.copy(operator = Condition.GreaterThan, valInt = Some(50), valString = None)
          val result = Condition.matches(51, c)
          result must === (true)
        }
