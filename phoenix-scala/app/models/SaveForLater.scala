...
class SaveForLaters(tag: Tag) extends FoxTable[SaveForLater](tag, "save_for_later") {
  ...
  def * =
    (id, accountId, variantId, createdAt) <> ((SaveForLater.apply _).tupled, SaveForLater.unapply)
...
object SaveForLaters
  ...
  def find(accountId: Int, variantId: Int): QuerySeq =
    filter(_.accountId === accountId).filter(_.variantId === variantId)
}
