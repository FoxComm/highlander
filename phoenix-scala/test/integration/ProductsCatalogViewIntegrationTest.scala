...
object ProductsCatalogViewIntegrationTest{
  ...
  case class ViewAlbum(name:String, images:Option[Seq[ViewImage]]){
    def this( payload: CreateAlbumPayload) = this(payload.name, payload.images.map(images ⇒ images.map(new ViewImage
    (_))))
  }
}

class ProductsCatalogViewIntegrationTest extends IntegrationTestBase with HttpSupport with AutomaticAuth {
  ...
  case class ProductAlbumsFromDatabase(product:Product){
    ...
    def usingProductCatalogView:List[ViewAlbum] =
      parseJsonColumnValues(sql"select albums from products_catalog_view where product_id = ${product.formId}")

    def usingProductAlbumLinksView:List[ViewAlbum] =
      parseJsonColumnValues(sql"select albums from product_album_links_view where product_id = ${product.id}")

    def usingAlbumSearchView: List[ViewAlbum] = {
      val albumIds = ProductAlbumLinks.filterLeft(product).map(_.rightId).result.gimme

      val columnValues = sql"select name, images from album_search_view where album_id in (select right_id from product_album_links where left_id = ${product.id})"
        .as[(String, Option[String])].gimme
      columnValues.map { case (name, images) ⇒
  ...
  "album-related views should be updated on" - {
    "album created" in new Fixture {
      ...
      val expectedAlbums: Seq[ViewAlbum] = Seq(new ViewAlbum(defaultAlbumPayload), new ViewAlbum(payload))
  ...
  trait Fixture {
    ...
    val (album, albumImages, product, sku) = (for {
      fullAlbum ← * <~ ObjectUtils.insertFullObject(defaultAlbumPayload.formAndShadow, ins ⇒ Albums.create(
      ...
      product ← * <~ Mvp.insertProductWithExistingSkus(ctx.id, SimpleProduct(title = "Test Product", active = true,
        description = "Test product description"), Seq(sku))

      _ ← * <~ ProductAlbumLinks.create(ProductAlbumLink(leftId = product.id, rightId = fullAlbum.model.id))
