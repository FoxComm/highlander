...
class CordsIntegrationTest extends IntegrationTestBase with TestObjectContext {
  ...
  "should not override cart's reference_number" in {
    val cart = Carts.create(Factories.cart.copy(referenceNumber = "foo")).gimme
    val cord = Cords.result.headOption.gimme.value
    cord.referenceNumber must === ("foo")
    cord.isCart mustBe true
  }

  "should generate and increment reference_number if empty" in {
    (1 to 3).map { i ⇒
  ...
  "cord should be updated and cart should be deleted on order creation" in {
