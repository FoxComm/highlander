...
import payloads.CategoryPayloads.{CreateFullCategory, UpdateFullCategory}
...
import payloads.NotePayloads.{CreateNote, UpdateNote}
import payloads.OrderPayloads._
import payloads.PaymentPayloads._
import payloads.ProductPayloads._
import payloads.SharedSearchPayloads.{SharedSearchAssociationPayload, SharedSearchPayload}
...
trait PhoenixAdminApi extends HttpSupport {
  ...
  case class customersApi(id: Int) {
    ...
    def disable(payload: ToggleCustomerDisabled): HttpResponse =
      POST(s"$customerPath/disable", payload)

    def blacklist(payload: ToggleCustomerBlacklisted): HttpResponse =
