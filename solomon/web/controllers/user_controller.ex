defmodule Solomon.UserController do
  ...
  def sign_in(conn, %{"org" => org, "email" => raw_email, "password" => pass}) do
    scope_id =
      Repo.get_by(Solomon.Organization, name: org)
      |> Map.fetch!(:scope_id)
  ...
  defp insert_and_relate(user_params) do
    ...
    |> Multi.run(:user, fn %{account: account} ->
      ...
      user_cs =
        User.changeset(%User{}, params_with_account)
        |> check_new_email
    ...
    |> Multi.run(:account_access_method, fn %{account: account, user: user} ->
      aam_cs =
        AccountAccessMethod.changeset(%AccountAccessMethod{}, %{
          "name" => "login",
          "hashed_password" =>
            user_params
            |> Map.fetch!("password")
  ...
  defp check_updated_email(changeset, id) do
    if Map.has_key?(changeset.changes, "email") &&
         Validation.email_is_taken(changeset.changes.email, id) do
      Ecto.Changeset.add_error(changeset, :email, "Email is already taken")
    else
      changeset
    end
