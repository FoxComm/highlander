...
select
    ...
    then
        ...
        json_agg(
            (s.code, 
                f.attributes->>(ss.attributes->'title'->>'ref'), 
                f.attributes->(ss.attributes->'price'->>'ref')->>'value')::export_skus)
    end as items
from customers as c
...
left join object_forms as f on s.form_id = f.id
left join object_shadows as ss on oli_skus.sku_shadow_id = ss.id
group by c.id;
