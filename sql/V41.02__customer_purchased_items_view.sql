    ...
    then
        ...
        json_agg(
            (s.sku, 
                ...
                s.attributes->'price'->>(ss.attributes->>'price'))::export_skus)
    end as items
from customers as c
...
left join products as p on oli_skus.product_id = p.id
left join product_shadows as ps on oli_skus.product_shadow_id = ps.id
group by c.id;
