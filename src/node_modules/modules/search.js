/* @flow */

import { get } from 'lodash';
import { createReducer, createAction } from 'redux-act';
import createAsyncActions from './async-utils';
import { api } from 'lib/api';
import type { Product } from './products';

export type Search = {
  term: string;
  results: Array<Product>;
}

const INITIAL_STATE:Search = {
  term: '',
  isActive: false,
  results: [],
};

function fullTextSearchPayload(searchString: string): Object {
  return {
    query: {
      bool: {
        must: [{
          match: {
            _all: {
              query: searchString,
              type: 'phrase_prefix',
              max_expansions: 3,
            },
          },
        }],
      },
    },
  };
}

/**
 * Generate search api call actions and reducer
 * Mocked for now
 */
function searchApiCall(searchString: string): global.Promise {
  const payload = fullTextSearchPayload(searchString);
  return api.post('/search/products_catalog_view/_search', payload);
}

const { fetch, ...searchActions } = createAsyncActions('search', searchApiCall);

/**
 * External actions
 */
export const setTerm = createAction('SET_TERM');
export const resetTerm = createAction('RESET_TERM');
export const toggleActive = createAction('TOGGLE_ACTIVE');
export { fetch };

const reducer = createReducer({
  [searchActions.fetchSucceeded]: (state, payload) => {
    return {
      ...state,
      results: payload,
    };
  },
  [setTerm]: (state, payload) => {
    return {
      ...state,
      term: payload,
    };
  },
  [resetTerm]: (state) => {
    return {
      ...state,
      term: '',
    };
  },
  [toggleActive]: (state) => {
    return {
      ...state,
      isActive: !get(state, 'isActive', false),
    };
  },
}, INITIAL_STATE);

export default reducer;
