...
export function initAddressData(kind: AddressKindType): Function {
  return (dispatch, getState) => {
    ...
    const addr = getState().cart.shippingAddress;
    ...
    if (addr && addr.region) {
      dispatch(fetchCountry(addr.region.countryId)).then(() => {
        const countryInfo = getState().countries.details[addr.region.countryId];

        uiAddressData = _.pick(addr, ['name', 'address1', 'address2', 'city', 'zip', 'phoneNumber']);
        uiAddressData.country = countryInfo;
        uiAddressData.state = _.find(countryInfo.regions, {id: addr.region.id});
...
export function saveShippingAddress(): Function {
  return (dispatch, getState) => {
    const addr = getState().checkout.shippingAddress;
    const payload = _.pick(addr, ['name', 'address1', 'address2', 'city', 'zip', 'phoneNumber']);
    payload.regionId = addr.state.id;
...
export function saveGiftCard(code: string): Function {
  return () => {
    const payload = { code, amount: 1000000 };
