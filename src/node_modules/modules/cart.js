...
function submitChange(payload, dispatch) {
  return phoenix.post('/v1/my/cart/line-items', payload).then(payload => {
    const skus = getLineItems(payload.result);
    dispatch(changeCart(skus));
  });
}
...
export function addLineItem(id, quantity) {
  return (dispatch, getState) => {
    ...
    return submitChange(newLineItems, dispatch);
  };
}
...
function fetchMyCart(): global.Promise {
  return phoenix.get(`/v1/my/cart`).then(result => getLineItems(result));
}
