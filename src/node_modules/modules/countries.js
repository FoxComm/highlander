...
function apiFetchCountries() {
  return phoenix.get('/v1/public/countries');
}

function apiFetchCountry(countryId) {
  return phoenix.get(`/v1/public/countries/${countryId}`);
}
...
const reducer = createReducer({
  [countriesActions.fetchSucceeded]: (state, result) => {
    return assoc(state,
      'list', result
    );
  },
  [countryActions.fetchSucceeded]: (state, result) => {
    return assoc(state,
