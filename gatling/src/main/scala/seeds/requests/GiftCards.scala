...
object GiftCards {
  ...
  private val reasonFeeder = dbFeeder(s"""select id as "reasonId" from reasons where reason_type = '$reasonType'""")

  val createGiftCard = http("Create gift card")
    .post("/v1/gift-cards")
    .body(StringBody(session ⇒ json(GiftCardCreateByCsr(
  ...
  val payWithGc = feed(reasonFeeder.random)
    .exec(createGiftCard)
    .exec(payWithGiftCard)
}
