...
func (suite *shipmentControllerTestSuite) Test_GetShipmentsByReferenceNumbers_Found_ReturnsRecords() {
	...
	address1 := suite.getTestAddress1(uint(1))
	address2 := suite.getTestAddress2(uint(2))
	...
	suite.assert.Equal(region1.ID, shipments[1].Address.Region.ID)
	suite.assert.Equal(country1.ID, shipments[0].Address.Region.CountryID)
	suite.assert.Equal(country1.ID, shipments[1].Address.Region.CountryID)
...
func (suite *shipmentControllerTestSuite) Test_CreateShipment_ReturnsRecord() {
	...
	payload.Address = payloads.Address{address1.Name,
		payloads.Region{address1.Region.ID, address1.Region.Name, address1.Region.Country.ID, address1.Region.Country.Name},
		address1.City, address1.Zip, address1.Address1, &address1.Address2.String, address1.PhoneNumber}
	shipmentLineItem1 := suite.getTestShipmentLineItem1(uint(1), shipment1.ID)
	shipmentLineItem2 := suite.getTestShipmentLineItem2(uint(2), shipment1.ID)
	...
	suite.assert.Equal(address1.Region.ID, shipment.Address.Region.ID)
	suite.assert.Equal(address1.Region.Country.ID, shipment.Address.Region.CountryID)
...
func (suite *shipmentControllerTestSuite) getTestAddress1(id uint) *models.Address {
	return &models.Address{gormfox.Base{ID: id}, "Home address", uint(1),
		*suite.getTestRegion1(), "Texas", "75231",
		"Some st, 335", sql.NullString{String: "", Valid: false}, "19527352893"}
}

func (suite *shipmentControllerTestSuite) getTestAddress2(id uint) *models.Address {
	return &models.Address{gormfox.Base{ID: id}, "Another address", uint(2),
		*suite.getTestRegion1(), "Florida", "31223",
...
func (suite *shipmentControllerTestSuite) getTestCountry1() *models.Country {
