defmodule Geronimo.Validation do
  ...
  def get_validations(spec) do
    ...
    |> Enum.map(fn {k, v} ->
      {k,
       validates(Keyword.fetch!(v, :type) |> String.to_atom()) ++
  ...
  def validates(:integer), do: [format: [with: ~r/^[0-9]*\z/, message: "only digits accepted"]]
  def validates(:text), do: []
  def validates(:string), do: []
  def validates(:array), do: [by: [function: &is_list/1, allow_blank: true]]

  def validates(:float),
    do: [format: [with: ~r/^\d+(?:\.\d+)?\z/, message: "should be like 123.45"]]
