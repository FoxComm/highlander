  ...
  defmacro __using__(_) do
    quote do
      ...
      def get(id, scope \\ "1") do
        try do
          case Repo.one(
                 from(m in __MODULE__, where: fragment("scope @> ?", ^scope), where: m.id == ^id)
               ) do
            nil -> {:error, "Not found"}
      ...
      def get_specific_version(id, version, scope) do
        datetime =
          Timex.parse!(version, "%FT%T.%fZ", :strftime)
      ...
      def update(id, params, scope) when is_map(params) do
        case get(id, scope) do
          {:ok, row} ->
            changeset(row, params)
            |> apply_change()

          {:error, _} ->
            raise %NotFoundError{
              message:
                "#{Inflex.singularize(table()) |> Inflex.camelize()} with id #{id} not found"
            }
        end
      end
      ...
      def format_version(data) do
        ...
        d =
          if Map.has_key?(v, :schema_version) do
