...
class GiftCardNotesIntegrationTest extends IntegrationTestBase with HttpSupport with AutomaticAuth {
  ...
  "POST /v1/notes/gift-card/:code" - {
    "can be created by an admin for a gift card" in new Fixture {
      val response = POST(s"v1/notes/gift-card/${giftCard.code}", CreateNote(body = "Hello, FoxCommerce!"))
  ...
  "PATCH /v1/notes/gift-card/:code/:noteId" - {
    ...
    "can update the body text" in new Fixture {
      ...
      val response = PATCH(s"v1/notes/gift-card/${giftCard.code}/${rootNote.id}", UpdateNote(body = "donkey"))
      response.status must === (StatusCodes.OK)
  ...
  "DELETE /v1/notes/gift-card/:code/:noteId" - {
    ...
    "can soft delete note" in new Fixture {
      val createResp = POST(s"v1/notes/gift-card/${giftCard.code}", CreateNote(body = "Hello, FoxCommerce!"))
