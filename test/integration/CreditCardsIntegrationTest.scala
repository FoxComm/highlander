...
class CreditCardsIntegrationTest
  ...
  "POST /v1/customers/:id/payment-methods/credit-cards (admin auth)" - {
    ...
    "creates cards for different customers correctly" in {
      ...
      val response1 =
        POST(s"v1/customers/${customer1.id}/payment-methods/credit-cards", thePayload)
      response1.status must === (StatusCodes.OK)
      val response2 =
  ...
  private def customerSourceMap(customer: Customer) =
    Map("description" → "FoxCommerce", "email" → customer.email.value, "source" → tokenStripeId)
