...
class JavaTimeMapperTest extends IntegrationTestBase with DbTestSupport {
  ...
  "java.time mapper" - {
    ...
    val ddl   = sqlu"create table java_time_test (id bigint primary key, deleted_at timestamp without time zone)"

    "writes instants correctly" ignore {
      val originalInstant: Instant = Instant.now()

      val (_, timestampAfterRoundtrip) = db.run((for {
        schema ← ddl
        insert ← query += ((1, originalInstant))
        read   ← query.filter(_.id === 1L).result.head
        _      ← query.schema.drop
      } yield read).transactionally).futureValue
    ...
    "reads instants correctly" in {
      val (_, timestampAfterRoundtrip) = db.run((for {
        ...
        read   ← query.filter(_.id === 1L).result.head
        _      ← query.schema.drop
      } yield read).withPinnedSession).futureValue

      timestampAfterRoundtrip must === (ZonedDateTime.of(2015, 7, 1, 15, 17, 38, 0, ZoneId.of("UTC")).toInstant)
    }
  }
}
