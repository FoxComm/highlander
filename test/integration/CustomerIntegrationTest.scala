...
class CustomerIntegrationTest
  ...
  def responseItems = {
    val insertCustomers = (1 to numOfResults).map { _ ⇒
      generateCustomer
    }
    val dbio = for {
  ...
  def responseItemsSort(items: IndexedSeq[CustomerResponse.Root]) =
    items.sortBy(_.name)
  ...
  "Customer" - {
    "accounts are unique based on email, non-guest, and active" in {
      ...
      val xor = swapDatabaseFailure(Customers.create(stub).run())(
          (NotUnique, failure)).futureValue
    ...
    "accounts are NOT unique for guest account and email" in {
      ...
      val customers =
        (1 to 3).map(_ ⇒ Customers.create(stub).futureValue.rightVal)
  ...
  "GET /v1/customers" - {
    "lists customers" in new Fixture {
      ...
      val customerRoot =
        CustomerResponse.build(customer, shippingRegion = region)

      response.status must ===(StatusCodes.OK)
      response.ignoreFailuresAndGiveMe[Seq[CustomerResponse.Root]] must ===(
          Seq(customerRoot))
    }
    ...
    "customer listing shows valid billingRegion" in new CreditCardFixture {
      val billRegion =
        Regions.findOneById(creditCard.regionId).run().futureValue

      val response = GET(s"$uriPrefix")
      val customerRoot = CustomerResponse.build(
          customer, shippingRegion = region, billingRegion = billRegion)
  ...
  "POST /v1/customers" - {
    ...
    "fails if email is already in use" in new Fixture {
      val response = POST(s"v1/customers",
                          CreateCustomerPayload(email = customer.email,
                                                name = Some("test")))
  ...
  "GET /v1/customers/:customerId" - {
    ...
    "empty phone number is resolved from" - {
      ...
      "last shipping address" in {
        ...
        val defaultAddress =
          Factories.address.copy(isDefaultShipping = true, phoneNumber = None)

        val orders = Seq(
            Factories.order.copy(state = Order.Shipped,
                                 referenceNumber = "ABC-1"),
            Factories.order.copy(state = Order.Shipped,
                                 referenceNumber = "ABC-2")
        )
        ...
        val (customer, region, shipments) = (for {
          customer ← * <~ Customers.create(
                        Factories.customer.copy(phoneNumber = None))
          address ← * <~ Addresses.create(
                       defaultAddress.copy(customerId = customer.id))
          region ← * <~ Regions.findOneById(address.regionId).toXor
          ordersSeq ← * <~ sequence(orders.map(o ⇒
                               DbResultT(Orders.create(
                                       o.copy(customerId = customer.id)))))
          addresses ← * <~ sequence(
                         shippingAddresses(ordersSeq.zip(phoneNumbers)).map(a ⇒
                               DbResultT(OrderShippingAddresses.create(a))))
          shipments ← * <~ sequence(addresses.map(address ⇒
                               DbResultT(
                                   Shipments.create(Factories.shipment.copy(
        ...
        def updateShipmentTime(s: Shipment, newTime: Instant ⇒ Instant): Unit =
          Shipments
            .update(s,
                    s.copy(updatedAt = s.updatedAt.map(time ⇒ newTime(time))))
            .run
            .futureValue

        def runTest(expectedPhone: String) = {
          val response = GET(s"$uriPrefix/${customer.id}")
          val expectedCustomer = CustomerResponse.build(
              customer.copy(phoneNumber = expectedPhone.some),
              shippingRegion = region)
    ...
    "fetches customer info with lastOrderDays value" in new CartFixture {
      val response = GET(s"$uriPrefix/${customer.id}")
      val expectedCustomer = CustomerResponse.build(
          customer, shippingRegion = region, lastOrderDays = None)
      ...
      Orders
        .update(
            order,
            order.copy(placedAt = Instant.now.minus(1, ChronoUnit.DAYS).some))
        .run
        .futureValue
        .rightVal
    ...
    "ranking" - {
      "customer must be with rank" in new FixtureForRanking {
        ...
        sqlu"UPDATE orders SET state = 'shipped' WHERE id = ${order.id}"
          .run()
          .futureValue
        sqlu"UPDATE rmas SET state = 'complete' WHERE id = ${orderPayment.id}"
          .run()
          .futureValue
        ...
        val rank = CustomersRanks
          ...
          .run()
          .futureValue
        val rank2 = CustomersRanks
          .findById(customer2.id)
          .extract
          .result
          .head
          .run()
  ...
  "POST /v1/customers/:customerId/activate" - {
    "fails if email is already in use by non-guest user" in new Fixture {
      val newUserResponse = POST(s"v1/customers",
                                 CreateCustomerPayload(email = customer.email,
                                                       isGuest = Some(true)))
    ...
    "sucessfully activate non-guest user" in new Fixture {
      val newUserResponse =
        POST(s"v1/customers",
             CreateCustomerPayload(email = "guest@example.com",
                                   isGuest = Some(true)))
  ...
  "POST /v1/customers/:customerId/disable" - {
    "toggles the isDisabled flag on a customer account" in new Fixture {
      ...
      val disableResp = POST(s"$uriPrefix/${customer.id}/disable",
                             ToggleCustomerDisabled(true))
      disableResp.status must ===(StatusCodes.OK)
      disableResp.as[CustomerResponse.Root].disabled must ===(true)

      val enableResp = POST(s"$uriPrefix/${customer.id}/disable",
                            ToggleCustomerDisabled(false))
    ...
    "fails if customer not found" in new Fixture {
      val response =
        POST(s"$uriPrefix/999/disable", ToggleCustomerDisabled(true))
    ...
    "disable already disabled account is ok (overwrite behaviour)" in new Fixture {
      val updated = rightValue(Customers
            .update(customer, customer.copy(isDisabled = true))
            .futureValue)
      updated.isDisabled must ===(true)

      val response = POST(s"$uriPrefix/${customer.id}/disable",
                          ToggleCustomerDisabled(true))
      response.status must ===(StatusCodes.OK)
  ...
  "POST /v1/customers/:customerId/blacklist" - {
    "toggles the isBlacklisted flag on a customer account" in new Fixture {
      ...
      val responseAdd = POST(s"$uriPrefix/${customer.id}/blacklist",
                             ToggleCustomerBlacklisted(true))
      responseAdd.status must ===(StatusCodes.OK)
      responseAdd.as[CustomerResponse.Root].isBlacklisted must ===(true)

      val responseRemove = POST(s"$uriPrefix/${customer.id}/blacklist",
                                ToggleCustomerBlacklisted(false))
    ...
    "fails if customer not found" in new Fixture {
      val response =
        POST(s"$uriPrefix/999/blacklist", ToggleCustomerBlacklisted(true))
    ...
    "blacklist already blacklisted account is ok (overwrite behaviour)" in new Fixture {
      val updated = rightValue(Customers
            .update(customer, customer.copy(isBlacklisted = true))
            .futureValue)
      updated.isBlacklisted must ===(true)

      val response = POST(s"$uriPrefix/${customer.id}/blacklist",
                          ToggleCustomerBlacklisted(true))
      response.status must ===(StatusCodes.OK)
  ...
  "GET /v1/customers/:customerId/payment-methods/credit-cards" - {
    "shows customer's credit cards only in their wallet" in new CreditCardFixture {
      val deleted = CreditCards
        .create(creditCard.copy(id = 0, inWallet = false))
        .run()
        .futureValue
        .rightVal

      val response =
        GET(s"$uriPrefix/${customer.id}/payment-methods/credit-cards")
      val cc = response.as[Seq[CardResponse]]
      val ccRegion =
        Regions.findOneById(creditCard.regionId).run().futureValue.value

      response.status must ===(StatusCodes.OK)
      cc must have size 1
      cc.head must ===(
          responses.CreditCardsResponse.build(creditCard, ccRegion))
  ...
  "POST /v1/customers/:customerId/payment-methods/credit-cards/:creditCardId/default" - {
    ...
    "successfully replaces an existing default credit card" in new Fixture {
      val default = CreditCards
        .create(Factories.creditCard.copy(isDefault = true,
                                          customerId = customer.id))
        .run()
        .futureValue
        .rightVal
      val nonDefault = CreditCards
        .create(Factories.creditCard.copy(isDefault = false,
                                          customerId = customer.id))
        .run()
        .futureValue
        .rightVal
    ...
    "fails when the credit card doesn't exist" in new Fixture {
      val payload = ToggleDefaultCreditCard(isDefault = true)
      val response = POST(
          s"$uriPrefix/${customer.id}/payment-methods/credit-cards/99/default",
          payload)
  ...
  "DELETE /v1/customers/:customerId/payment-methods/credit-cards/:creditCardId" - {
    "deletes successfully if the card exists" in new CreditCardFixture {
      ...
      val response = DELETE(
          s"$uriPrefix/${customer.id}/payment-methods/credit-cards/${creditCard.id}")
      val deleted =
        CreditCards.findOneById(creditCard.id).run().futureValue.value
  ...
  "PATCH /v1/customers/:customerId/payment-methods/credit-cards/:creditCardId" - {
    "when successful" - {
      ...
      "creates a new version of the edited card in the wallet" in new CreditCardFixture {
        ...
        val response = PATCH(
            s"$uriPrefix/${customer.id}/payment-methods/credit-cards/${creditCard.id}",
            payload)
        val (newVersion :: Nil) = CreditCards
          .filter(_.parentId === creditCard.id)
          .result
          .run()
          .futureValue
          .toList
      ...
      "copies an existing address book entry to the creditCard" in new CreditCardFixture {
        val payload =
          EditCreditCard(holderName = Some("Bob"), addressId = address.id.some)
        val response = PATCH(
            s"$uriPrefix/${customer.id}/payment-methods/credit-cards/${creditCard.id}",
            payload)
        val (newVersion :: Nil) = CreditCards
        ...
        response.status must ===(StatusCodes.OK)
        numAddresses must ===(1)
        (newVersion.zip, newVersion.regionId) must ===(
      ...
      "creates a new address book entry if a full address was given" in new CreditCardFixture {
        ...
        val payload =
          EditCreditCard(holderName = Some("Bob"),
                         address =
    ...
    "fails if the card cannot be found" in new CreditCardFixture {
      ...
      val response =
        PATCH(s"$uriPrefix/${customer.id}/payment-methods/credit-cards/99",
              payload)
    ...
    "fails if the card is not inWallet" in new CreditCardFixture {
      ...
      CreditCardManager
        .deleteCreditCard(customer.id, creditCard.id, Some(admin))
        .futureValue
      ...
      response.status must ===(StatusCodes.BadRequest)
      response.error must ===(
          CannotUseInactiveCreditCard(creditCard).description)
    }
    ...
    "fails if stripe returns an error" in new CreditCardFixture {
      ...
      when(stripeApi.updateExternalAccount(m.any(), m.any(), m.any()))
        .thenReturn(
          Result.failure(StripeFailure(
                  new CardException("Your card's expiration year is invalid",
                                    "X_REQUEST_ID: 1",
                                    "invalid_expiry_year",
                                    "exp_year",
                                    null,
  ...
  "GET /v1/customers/searchForNewOrder?term=:term" - {
    "successfully search by term" in new Fixture {
      val response =
        GET(s"$uriPrefix/searchForNewOrder?term=${customer.email.drop(2)}")
      response.status must ===(StatusCodes.OK)

      response.ignoreFailuresAndGiveMe[Seq[CustomerResponse.Root]].size must ===(
          1)
    }
  }
  ...
  trait FixtureForRanking extends CreditCardFixture {
    val (order, orderPayment, customer2) = (for {
      customer2 ← * <~ Customers.create(Factories.customer.copy(
                         email = "second@example.org", name = Some("second")))
      order ← * <~ Orders.create(
