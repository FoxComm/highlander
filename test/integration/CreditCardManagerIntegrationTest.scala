...
class CreditCardManagerIntegrationTest
  ...
  "CreditCardManagerTest" - {
    "POST /v1/customers/:id/payment-methods/credit-cards" - {
      ...
      def payloadWithFullAddress(p: CreateCreditCard,
                                 a: Address): CreateCreditCard = {
      ...
      "successfully" - {
        "copies an existing address to the new creditCard" ignore new AddressFixture {
          val payload =
            payloadStub.copy(addressId = Some(address.id), isDefault = true)
          val response =
            POST(s"v1/customers/${customer.id}/payment-methods/credit-cards",
                 payload)
          val (cc :: Nil) =
            CreditCards.filter(_.customerId === customer.id).futureValue.toList
          ...
          (cc.expYear, cc.expMonth) must ===(
              (payload.expYear, payload.expMonth))
        ...
        "creates a new address in the book and copies it to the new creditCard" ignore new Fixture {
          ...
          val payload =
            payloadWithFullAddress(payloadStub.copy(isDefault = true), a)
        ...
        "uses an existing stripe customerId when it exists" ignore new AddressFixture {
          ...
          val response =
            POST(s"v1/customers/${customer.id}/payment-methods/credit-cards",
                 payload)
          val cards =
            CreditCards.filter(_.customerId === customer.id).futureValue
      ...
      "fails" - {
        ...
        "if the addressId cannot be found in address book" ignore new Fixture {
          ...
          response.status must ===(StatusCodes.BadRequest)
          response.error must contain(
              NotFoundFailure404(Address, 1).description)
          cards mustBe 'empty
        }
        ...
        "if Stripe's CVC check fails" ignore new AddressFixture {
          val payload =
            payloadStub.copy(cardNumber = StripeSupport.incorrectCvc,
                             addressId = Some(1))
          val response =
            POST(s"v1/customers/${customer.id}/payment-methods/credit-cards",
                 payload)
        ...
        "if customer cannot be found" ignore {
          ...
          val response =
            POST(s"v1/customers/99/payment-methods/credit-cards", payload)
          val cards = CreditCards.futureValue
