...
class StoreCreditIntegrationTest extends IntegrationTestBase {
  ...
  "StoreCreditTest" - {
    ...
    "updates availableBalance if auth adjustment is created + cancel handling" in new Fixture {
      val adjustment = StoreCredits
        .auth(storeCredit, Some(payment.id), 1000)
        .run()
        .futureValue
        .rightVal

      val updatedStoreCredit =
        StoreCredits.findOneById(storeCredit.id).run().futureValue.value
      updatedStoreCredit.availableBalance must ===(
          storeCredit.availableBalance - 1000)

      StoreCreditAdjustments.cancel(adjustment.id).run().futureValue
      val canceledStoreCredit =
        StoreCredits.findOneById(storeCredit.id).run().futureValue.value
      canceledStoreCredit.availableBalance must ===(
          storeCredit.availableBalance)
    }

    "updates availableBalance and currentBalance if capture adjustment is created + cancel handling" in new Fixture {
      val adjustment = StoreCredits
        .capture(storeCredit, Some(payment.id), 1000)
        .run()
        .futureValue
        .rightVal
      ...
      updatedStoreCredit.availableBalance must ===(
          storeCredit.availableBalance - 1000)
      updatedStoreCredit.currentBalance must ===(
          storeCredit.currentBalance - 1000)
  ...
  trait Fixture {
    val (customer, origin, storeCredit, payment) = (for {
      ...
      sc ← * <~ StoreCredits.create(Factories.storeCredit.copy(
                  customerId = customer.id, originId = origin.id))
      ...
      (customer, origin, sCredit.value, payment)).runTxn().futureValue.rightVal
  }
}
