...
class InventorySummaryIntegrationTest extends IntegrationTestBase {
  ...
  "Trigger calculates AFS" - {
    ...
    "for sellable summary" in {
      val unsaved = SellableInventorySummary(
          onHand = 10, onHold = 1, reserved = 2, safetyStock = 3)
      val summary =
        SellableInventorySummaries.create(unsaved).run().futureValue.rightVal
      ...
      val updated = SellableInventorySummaries
        .findOneById(summary.id)
        .run()
        .futureValue
        .value
    ...
    "for preorder summary" in {
      val unsaved =
        PreorderInventorySummary(onHand = 10, onHold = 1, reserved = 2)
      val summary =
        PreorderInventorySummaries.create(unsaved).run().futureValue.rightVal
      ...
      PreorderInventorySummaries
        ...
        .rightVal
      // FIXME https://github.com/FoxComm/phoenix-scala/issues/821
      val updated = PreorderInventorySummaries
        .findOneById(summary.id)
        .run()
        .futureValue
        .value
    ...
    "for backorder summary" in {
      val unsaved =
        BackorderInventorySummary(onHand = 10, onHold = 1, reserved = 2)
      val summary =
        BackorderInventorySummaries.create(unsaved).run().futureValue.rightVal
      summary.availableForSale must ===(7)
      BackorderInventorySummaries
        ...
        .rightVal
      // FIXME https://github.com/FoxComm/phoenix-scala/issues/821
      val updated = BackorderInventorySummaries
        .findOneById(summary.id)
        .run()
        .futureValue
        .value
    ...
    "for nonsellable summary" in {
      val unsaved =
        NonSellableInventorySummary(onHand = 10, onHold = 1, reserved = 2)
      val summary = NonSellableInventorySummaries
        .create(unsaved)
        .run()
        .futureValue
        .rightVal
      ...
      NonSellableInventorySummaries
        ...
        .rightVal
      // FIXME https://github.com/FoxComm/phoenix-scala/issues/821
      val updated = NonSellableInventorySummaries
        .findOneById(summary.id)
        .run()
        .futureValue
        .value
