...
class RmaPaymentsIntegrationTest
  ...
  "gift cards" - {
    ...
    "POST /v1/rmas/:ref/payment-methods/gift-cards" - {
      "successfully creates gift card as payment method" in new Fixture {
        ...
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/payment-methods/gift-cards",
               payload)
    ...
    "DELETE /v1/rmas/:ref/payment-methods/gift-cards" - {
      "successfully deletes a giftCard" in new Fixture {
        ...
        val response =
          DELETE(s"v1/rmas/${rma.referenceNumber}/payment-methods/gift-cards")
        response.status must ===(StatusCodes.OK)
  ...
  "store credit" - {
    ...
    "POST /v1/rmas/:ref/payment-methods/store-credit" - {
      "successfully creates store credit as payment method" in new Fixture {
        ...
        val response =
          POST(s"v1/rmas/${rma.refNum}/payment-methods/store-credit", payload)
        response.status must ===(StatusCodes.OK)
      ...
      "fails if the amount is less than zero" in new Fixture {
        val payload = RmaPaymentPayload(amount = -10)
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/payment-methods/store-credit",
               payload)
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===("Amount got -10, expected more than 0")
      }
      ...
      "fails if the RMA is not found" in new Fixture {
        ...
        val response =
          POST(s"v1/rmas/${notFound.refNum}/payment-methods/store-credit",
               payload)
        response.status must ===(StatusCodes.NotFound)
        response.error must ===(
            NotFoundFailure404(Rma, notFound.refNum).description)
      }
    }
    ...
    "DELETE /v1/rmas/:ref/payment-methods/store-credit" - {
      "successfully deletes store credit payment" in new Fixture {
        ...
        val response = DELETE(
            s"v1/rmas/${rma.referenceNumber}/payment-methods/store-credit")
        response.status must ===(StatusCodes.OK)
  ...
  "credit cards" - {
    ...
    "POST /v1/rmas/:ref/payment-methods/credit-cards" - {
      "succeeds" in new Fixture {
        val payload = RmaPaymentPayload(amount = 50)
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/payment-methods/credit-cards",
               payload)
        response.status must ===(StatusCodes.OK)
      ...
      "fails if the RMA is not found" in new Fixture {
        ...
        val response =
          POST(s"v1/rmas/99/payment-methods/credit-cards", payload)
        response.status must ===(StatusCodes.NotFound)
        response.error must ===(NotFoundFailure404(Rma, 99).description)
      }
    }
    ...
    "DELETE /v1/rmas/:ref/payment-methods/credit-cards" - {
      "successfully deletes an existing card" in new Fixture {
        ...
        val response = DELETE(
            s"v1/rmas/${rma.referenceNumber}/payment-methods/credit-cards")
        response.status must ===(StatusCodes.OK)
    ...
    val (rma, order, admin, customer) = (for {
      ...
      orderPayment ← * <~ OrderPayments.create(
                        Factories.orderPayment.copy(orderId = order.id,
                                                    ...
                                                    amount = None))
      rma ← * <~ Rmas.create(
               Factories.rma.copy(referenceNumber = "ABCD1234-11.1"))
