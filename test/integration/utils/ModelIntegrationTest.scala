...
class ModelIntegrationTest extends IntegrationTestBase {
  ...
  "New model create" - {
    "validates model" in {
      val failures = leftValue(Addresses
            .create(Factories.address.copy(zip = "totallyNotAValidZip"))
            .run()
            .futureValue)
      failures must ===(GeneralFailure(
              "zip must fully match regular expression '^\\d{5}(?:\\d{4})?$'").single)
    }
    ...
    "sanitizes model" in {
      val result = (for {
        ...
        address ← * <~ Addresses.create(Factories.address.copy(
                         zip = "123-45", customerId = customer.id))
    ...
    "catches exceptions from DB" in {
      val result = (for {
        customer ← * <~ Customers.create(Factories.customer)
        original ← * <~ Addresses.create(
                      Factories.address.copy(customerId = customer.id))
        copycat ← * <~ Addresses.create(
                     Factories.address.copy(customerId = customer.id))
  ...
  "Model update" - {
    ...
    "model refuses to update if FSM check fails" in {
      ...
      failure must ===(StateTransitionNotAllowed(
              origin.state, destination.state, origin.refNum).single)
    }
    ...
    "must update model successfully" in {
      ...
      val updated = Customers
        .update(customer, customer.copy(name = Some("Derp")))
        .run()
        .futureValue
        .rightVal
      Customers.findOneById(customer.id).run().futureValue.value must ===(
          updated)
    }
    ...
    "must run FSM check if applicable" in {
      ...
      val updateFailure = leftValue(Orders
            .update(order, order.copy(state = Order.Cart))
            .run()
            .futureValue)
      updateFailure must ===(StateTransitionNotAllowed(
              order.state, Order.Cart, order.refNum).single)
