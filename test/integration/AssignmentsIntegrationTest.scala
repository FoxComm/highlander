...
class AssignmentsIntegrationTest extends IntegrationTestBase
  ...
  "POST /v1/orders/:refNum/assignees" - {
    ...
    "extends response with errors if one of store admins is not found" in new Fixture {
      ...
      theResponse.errors.value.headOption.value mustBe NotFoundFailure404(StoreAdmin, nonExistentAdminId).description
    }
  ...
  "POST /v1/orders/assignees" - {
    ...
    "can be assigned to multiple orders with graceful error handling" in new BulkAssignmentFixture {
      val payload = BulkAssignmentPayload(entityIds = Seq(order1.refNum, order2.refNum, "NOPE"), storeAdminId = admin.id)
      ...
      val alreadyAssignedFailure = AlreadyAssignedFailure(Order, order1.refNum, admin.id).description
      val assertFailures = Map[String, String]("NOPE" → notFoundFailure, order1.refNum → alreadyAssignedFailure)
  ...
  "POST /v1/orders/assignees/delete" - {
    ...
    "can be unassigned from multiple orders with graceful error handling" in new BulkAssignmentFixture {
      ...
      val assertFailures = Map[String, String]("NOPE" → notFoundFailure, order2.refNum → notAssignedFailure)
  ...
  trait BulkAssignmentFixture {
    val (order1, order2, admin) = (for {
      ...
      assignee ← * <~ Assignments.create(Assignment(referenceType = Assignment.Order, referenceId = order1.id,
        storeAdminId = admin.id, assignmentType = Assignment.Assignee))
