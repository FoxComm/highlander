...
class StoreCreditIntegrationTest
  ...
  "StoreCredits" - {
    "POST /v1/customers/:id/payment-methods/store-credit" - {
      "when successful" - {
        "responds with the new storeCredit" in new Fixture {
          val payload =
            CreateManualStoreCredit(amount = 25, reasonId = scReason.id)
          val response =
            POST(s"v1/customers/${customer.id}/payment-methods/store-credit",
                 payload)
          ...
          val manual =
            StoreCreditManuals.findOneById(sc.originId).run().futureValue.value
      ...
      "succeeds with valid subTypeId" in new Fixture {
        val payload = CreateManualStoreCredit(amount = 25,
      ...
      "fails if subtypeId is not found" in new Fixture {
        ...
        response.error must ===(
            NotFoundFailure404(StoreCreditSubtype, 255).description)
      }

      "fails if the customer is not found" in {
        val payload = CreateManualStoreCredit(amount = 25, reasonId = 1)
        val response =
          POST(s"v1/customers/99/payment-methods/store-credit", payload)
    ...
    "GET /v1/customers/:id/payment-methods/store-credit" - {
      "returns list of store credits" in new Fixture {
        val response =
          GET(s"v1/customers/${customer.id}/payment-methods/store-credit")
        val storeCredits = Seq(storeCredit, scSecond)
        response.status must ===(StatusCodes.OK)

        val result =
          response.ignoreFailuresAndGiveMe[StoreCreditResponse.WithTotals]
        result.storeCredits.map(_.id).sorted must ===(
            storeCredits.map(_.id).sorted)
    ...
    "GET /v1/customers/:id/payment-methods/store-credit/transactions" - {
      "returns list of store credit transactions" in new Fixture {
        val response = GET(
            s"v1/customers/${customer.id}/payment-methods/store-credit/transactions")
        response.status must ===(StatusCodes.OK)

        val adjustments = response
          .ignoreFailuresAndGiveMe[Seq[StoreCreditAdjustmentsResponse.Root]]
      ...
      "returns not found when customer doesn't exist" in new Fixture {
        val response =
          GET(s"v1/customers/99/payment-methods/store-credit/transactions")
    ...
    "GET /v1/customers/:id/payment-methods/store-credit/total" - {
      "returns total available and current store credit for customer" in new Fixture {
        val response = GET(
            s"v1/customers/${customer.id}/payment-methods/store-credit/totals")
        response.status must ===(StatusCodes.OK)
    ...
    "GET /v1/store-credits/:id/transactions" - {
      ...
      "returns the list of adjustments with sorting and paging" in new Fixture {
        ...
        val adjustment2 =
          StoreCredits.auth(storeCredit, Some(payment.id), 1).run().futureValue
        val adjustment3 =
          StoreCredits.auth(storeCredit, Some(payment.id), 2).run().futureValue

        val response = GET(
            s"v1/store-credits/${storeCredit.id}/transactions?sortBy=-id&from=2&size=2")
        val adjustments = response
          .ignoreFailuresAndGiveMe[Seq[StoreCreditAdjustmentsResponse.Root]]
    ...
    "PATCH /v1/store-credits/:id" - {
      "successfully changes status from Active to OnHold and vice-versa" in new Fixture {
        val response = PATCH(s"v1/store-credits/${storeCredit.id}",
                             StoreCreditUpdateStateByCsr(state = OnHold))
        response.status must ===(StatusCodes.OK)

        val responseBack = PATCH(s"v1/store-credits/${storeCredit.id}",
                                 StoreCreditUpdateStateByCsr(state = Active))
      ...
      "successfully cancels store credit with provided reason, cancel adjustment is created" in new Fixture {
        ...
        adjustments.head.state must ===(
            StoreCreditAdjustment.CancellationCapture)
      }
      ...
      "successfully cancels store credit with zero balance" in new Fixture {
        ...
        // Update balance
        StoreCredits
          .update(storeCredit, storeCredit.copy(availableBalance = 0))
          .run()
          .futureValue
    ...
    "PATCH /v1/store-credits" - {
      "successfully changes statuses of multiple store credits" in new Fixture {
        ...
        val firstUpdated =
          StoreCredits.findOneById(storeCredit.id).run().futureValue
        firstUpdated.value.state must ===(StoreCredit.OnHold)

        val secondUpdated =
          StoreCredits.findOneById(scSecond.id).run().futureValue
    ...
    "POST /v1/customers/:customerId/payment-methods/store-credit/:id/convert" - {
      "successfully converts SC to GC" in new Fixture {
        val response = POST(
            s"v1/customers/${customer.id}/payment-methods/store-credit/${scSecond.id}/convert")
        response.status must ===(StatusCodes.OK)
        ...
        val redeemedSc =
          StoreCredits.filter(_.id === scSecond.id).one.run().futureValue.value
      ...
      "fails to convert when SC not found" in new Fixture {
        val response = POST(
            s"v1/customers/${customer.id}/payment-methods/store-credit/555/convert")
        response.status must ===(StatusCodes.NotFound)
        response.error must ===(
            NotFoundFailure404(StoreCredit, 555).description)
      }

      "fails to convert when customer not found" in new Fixture {
        val response = POST(
            s"v1/customers/666/payment-methods/store-credit/${scSecond.id}/convert")
        response.status must ===(StatusCodes.NotFound)
      ...
      "fails to convert inactive SC to GC" in new Fixture {
        StoreCredits
          ...
          .futureValue
        val updatedSc =
          StoreCredits.findActiveById(scSecond.id).one.run().futureValue.value

        val response = POST(
            s"v1/customers/${customer.id}/payment-methods/store-credit/${scSecond.id}/convert")
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            StoreCreditConvertFailure(updatedSc).description)
      }
    }
  }
  ...
  trait Fixture {
    val (admin,
      ...
      scSecond ← * <~ StoreCredits.create(Factories.storeCredit.copy(
                        originId = scOrigin.id, customerId = customer.id))
      payment ← * <~ OrderPayments.create(Factories.storeCreditPayment.copy(
                       orderId = order.id,
