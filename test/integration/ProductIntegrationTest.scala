...
class ProductIntegrationTest
  ...
  "GET v1/products/full/:context/:id/baked" - {
    "Return a product with multiple SKUs and variants" in new Fixture {
      val response =
        GET(s"v1/products/full/${context.name}/${product.formId}/baked")
      ...
      val variantMap =
        productResponse.variantMap.extract[Map[String, Seq[Int]]]
  ...
  trait Fixture {
    val simpleProd = SimpleProductData(
        title = "Test Product",
        code = "TEST",
    ...
    val variantsWithValues = Seq(
        SimpleCompleteVariant(SimpleVariant("Size"),
                              Seq(SimpleVariantValue("small", ""),
                                  SimpleVariantValue("large", ""))),
    ...
    val (context, product, skus, variants) = (for {
      ...
      // Create the product.
      product ← * <~ Mvp.insertProductWithExistingSkus(
                   context.id, simpleProd, skus)

      // Create the Variants and their Values.
      variantsAndValues ← * <~ DbResultT.sequence(
                             variantsWithValues.map { scv ⇒
                           Mvp.insertVariantWithValues(
                               context.id, product.shadowId, scv)
                         })

      variants ← * <~ variantsAndValues.map(_.variant)
      variantValues ← * <~ variantsAndValues.foldLeft(
                         Seq.empty[SimpleVariantValueData]) { (acc, item) ⇒
      ...
      skuMap ← * <~ DbResultT.sequence(skuValueMapping.map {
                case (code, colorName, sizeName) ⇒
                  ...
                  val colorValue =
                    variantValues.filter(_.name == colorName).head
                  val sizeValue = variantValues.filter(_.name == sizeName).head

                  for {
                    colorLink ← * <~ ObjectLinks.create(ObjectLink(
                                       leftId = selectedSku.shadowId,
                                       rightId = colorValue.shadowId,
                                       linkType = ObjectLink.SkuVariantValue))
                    sizeLink ← * <~ ObjectLinks.create(ObjectLink(
                                      leftId = selectedSku.shadowId,
                                      rightId = sizeValue.shadowId,
                                      linkType = ObjectLink.SkuVariantValue))
                  } yield (colorLink, sizeLink)
              })
    } yield (context, product, skus, variantsAndValues))
      .runTxn()
      .futureValue
      .rightVal
  }
