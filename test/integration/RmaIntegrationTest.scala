...
class RmaIntegrationTest
  ...
  "RMAs" - {
    ...
    "GET /v1/rmas/order/:refNum" - {
      ...
      "should return failure for non-existing order" in new Fixture {
        ...
        response.status must ===(StatusCodes.NotFound)
        response.error must ===(
            NotFoundFailure404(Order, "ABC-666").description)
      }
    }
    ...
    "PATCH /v1/rmas/:refNum" - {
      "successfully changes status of RMA" in new Fixture {
        val response = PATCH(s"v1/rmas/${rma.referenceNumber}",
                             RmaUpdateStatePayload(state = Processing))
        response.status must ===(StatusCodes.OK)
      ...
      "successfully cancels RMA with valid reason" in new Fixture {
        val payload =
          RmaUpdateStatePayload(state = Canceled, reasonId = Some(reason.id))
      ...
      "fails to cancel RMA if invalid reason provided" in new Fixture {
        val response =
          PATCH(s"v1/rmas/${rma.referenceNumber}",
                RmaUpdateStatePayload(state = Canceled, reasonId = Some(999)))
      ...
      "fails if refNum is not found" in new LineItemFixture {
        val response =
          PATCH(s"v1/rmas/ABC-666", RmaUpdateStatePayload(state = Processing))
        response.status must ===(StatusCodes.NotFound)
        response.error must ===(NotFoundFailure404(Rma, "ABC-666").description)
      }
    }
    ...
    "POST /v1/rmas" - {
      "successfully creates new RMA" in new Fixture {
        val response = POST(s"v1/rmas",
                            RmaCreatePayload(orderRefNum = order.refNum,
      ...
      "fails to create RMA with invalid order refNum provided" in new Fixture {
        val response = POST(s"v1/rmas",
                            RmaCreatePayload(orderRefNum = "ABC-666",
                                             rmaType = Rma.Standard))
        response.status must ===(StatusCodes.NotFound)
        response.error must ===(
            NotFoundFailure404(Order, "ABC-666").description)
    ...
    "POST /v1/rmas/:refNum/message" - {
      "successfully manipulates with message to the customer" in new Fixture {
        ...
        val responseClean = POST(s"v1/rmas/${rma.referenceNumber}/message",
                                 RmaMessageToCustomerPayload(message = ""))
      ...
      "fails if message is too long" in new Fixture {
        val payload = RmaMessageToCustomerPayload(message =
              List.fill(Rma.messageToCustomerMaxLength)("Yax").mkString)
        val response = POST(s"v1/rmas/99/message", payload)

        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            "Message length got 3000, expected 1000 or less")
      }
    }
    ...
    "POST /v1/rmas/:refNum/lock" - {
      "successfully locks an RMA" in new Fixture {
        ...
        val lockedRma =
          Rmas.findByRefNum(rma.referenceNumber).result.run().futureValue.head
      ...
      "refuses to lock an already locked RMA" in {
        ...
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            LockedFailure(Rma, rma.referenceNumber).description)
      }
  ...
  "RMA Line Items" - {
    ...
    "POST /v1/rmas/:refNum/line-items/skus" - {
      "successfully adds SKU line item" in new LineItemFixture {
        val payload = RmaSkuLineItemsPayload(sku = sku.code,
                                             quantity = 1,
                                             reasonId = rmaReason.id,
                                             isReturnItem = true,
                                             inventoryDisposition =
                                               RmaLineItem.Putaway)
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/line-items/skus", payload)
        response.status must ===(StatusCodes.OK)
      ...
      "fails if refNum is not found" in new LineItemFixture {
        val payload = RmaSkuLineItemsPayload(sku = "ABC-666",
                                             quantity = 1,
                                             reasonId = rmaReason.id,
                                             isReturnItem = true,
                                             inventoryDisposition =
    ...
    "DELETE /v1/rmas/:refNum/line-items/skus/:id" - {
      "successfully deletes SKU line item" in new LineItemFixture {
        ...
        val payload = RmaSkuLineItemsPayload(sku = sku.code,
                                             ...
                                             inventoryDisposition =
                                               RmaLineItem.Putaway)
        val updatedRma = RmaLineItemUpdater
        ...
        // Delete
        val response =
          DELETE(s"v1/rmas/${rma.referenceNumber}/line-items/skus/$lineItemId")
        response.status must ===(StatusCodes.OK)
        val root = response.as[RmaResponse.Root]
      ...
      "fails if line item ID is not found" in new LineItemFixture {
        val response =
          DELETE(s"v1/rmas/${rma.referenceNumber}/line-items/skus/666")
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            NotFoundFailure400(RmaLineItem, 666).description)
      }
    }
    ...
    "POST /v1/rmas/:refNum/line-items/gift-cards" - {
      "successfully adds gift card line item" in new LineItemFixture {
        val payload = RmaGiftCardLineItemsPayload(code = giftCard.code,
                                                  reasonId = rmaReason.id)
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/line-items/gift-cards",
               payload)
        response.status must ===(StatusCodes.OK)

        val root = response.as[RmaResponse.Root]
        root.lineItems.giftCards.headOption.value.giftCard.code must ===(
            giftCard.code)
      }

      "fails if refNum is not found" in new LineItemFixture {
        val payload = RmaGiftCardLineItemsPayload(code = "ABC-666",
                                                  reasonId = rmaReason.id)
      ...
      "fails if reason is not found" in new LineItemFixture {
        val payload =
          RmaGiftCardLineItemsPayload(code = "ABC-666", reasonId = 100)
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/line-items/gift-cards",
               payload)
    ...
    "DELETE /v1/rmas/:refNum/line-items/gift-cards/:id" - {
      "successfully deletes gift card line item" in new LineItemFixture {
        ...
        val updatedRma = RmaLineItemUpdater
          ...
          .rightVal
        val lineItemId =
          updatedRma.lineItems.giftCards.headOption.value.lineItemId

        // Delete
        val response = DELETE(
            s"v1/rmas/${rma.referenceNumber}/line-items/gift-cards/$lineItemId")
        response.status must ===(StatusCodes.OK)
        val root = response.as[RmaResponse.Root]
      ...
      "fails if line item ID is not found" in new LineItemFixture {
        val response =
          DELETE(s"v1/rmas/${rma.referenceNumber}/line-items/gift-cards/666")
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            NotFoundFailure400(RmaLineItem, 666).description)
      }
    ...
    "POST /v1/rmas/:refNum/line-items/shipping-costs" - {
      "successfully adds shipping cost line item" in new LineItemFixture {
        ...
        val response =
          POST(s"v1/rmas/${rma.referenceNumber}/line-items/shipping-costs",
               payload)
        response.status must ===(StatusCodes.OK)

        val root = response.as[RmaResponse.Root]
        root.lineItems.shippingCosts.headOption.value.shippingCost.id must ===(
            shipment.id)
      }
      ...
      "fails if refNum is not found" in new LineItemFixture {
        ...
        val response =
          POST(s"v1/rmas/ABC-666/line-items/shipping-costs", payload)
    ...
    "DELETE /v1/rmas/:refNum/line-items/shipping-costs/:id" - {
      "successfully deletes shipping cost line item" in new LineItemFixture {
        ...
        val updatedRma = RmaLineItemUpdater
          ...
          .rightVal
        val lineItemId =
          updatedRma.lineItems.shippingCosts.headOption.value.lineItemId

        // Delete
        val response = DELETE(
            s"v1/rmas/${rma.referenceNumber}/line-items/shipping-costs/$lineItemId")
        response.status must ===(StatusCodes.OK)
        val root = response.as[RmaResponse.Root]
      ...
      "fails if line item ID is not found" in new LineItemFixture {
        val response = DELETE(
            s"v1/rmas/${rma.referenceNumber}/line-items/shipping-costs/666")
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            NotFoundFailure400(RmaLineItem, 666).description)
      }
  ...
  trait Fixture {
    val (storeAdmin, customer, order, rma, reason) = (for {
      ...
      reason ← * <~ Reasons.create(
                  Factories.reason.copy(storeAdminId = storeAdmin.id))
    } yield
      (storeAdmin, customer, order, rma, reason)).runTxn().futureValue.rightVal
  }
  ...
  trait LineItemFixture extends Fixture {
    val (productContext, rmaReason, sku, giftCard, shipment) = (for {
      ...
      _ ← * <~ Factories.addSkusToOrder(
             Seq(sku.id), order.id, OrderLineItem.Cart)

      gcReason ← * <~ Reasons.create(
                    Factories.reason.copy(storeAdminId = storeAdmin.id))
      gcOrigin ← * <~ GiftCardManuals.create(GiftCardManual(
                        adminId = storeAdmin.id, reasonId = gcReason.id))
      giftCard ← * <~ GiftCards.create(
                    Factories.giftCard.copy(originId = gcOrigin.id,
                                            originType = GiftCard.RmaProcess))

      gcLineItem ← * <~ OrderLineItemGiftCards.create(OrderLineItemGiftCard(
                          orderId = order.id, giftCardId = giftCard.id))
      ...
      orderShippingMethod ← * <~ OrderShippingMethods.create(
                               OrderShippingMethod.build(
                                   order = order, method = shippingMethod))
      shipment ← * <~ Shipments.create(Factories.shipment)
    } yield (productContext, rmaReason, sku, giftCard, shipment))
      .runTxn()
      .futureValue
      .rightVal
  }
