...
class OrderNotesIntegrationTest extends IntegrationTestBase with HttpSupport with AutomaticAuth {
  ...
  "GET /v1/notes/order/:refNum" - {
    "can be listed" in new Fixture {
      List("abc", "123", "xyz").map { body ⇒
        OrderNoteManager.createOrderNote(order.refNum, storeAdmin, payloads.CreateNote(body = body)).futureValue
      }
  ...
  "PATCH /v1/notes/order/:refNum/:noteId" - {
    "can update the body text" in new Fixture {
      val rootNote = OrderNoteManager.createOrderNote(order.refNum, storeAdmin,
  ...
  "DELETE /v1/notes/order/:refNum/:noteId" - {
    "can soft delete note" in new Fixture {
      val note = rightValue(OrderNoteManager.createOrderNote(order.refNum, storeAdmin,
