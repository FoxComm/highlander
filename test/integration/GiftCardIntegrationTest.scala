...
class GiftCardIntegrationTest
  ...
  def responseItems = {
    ...
    } yield giftCards.map(GiftCardResponse.build(_)))
      .run()
      .futureValue
      .rightVal
      .toIndexedSeq
  ...
  "GiftCards" - {
    ...
    "POST /v1/gift-cards" - {
      "successfully creates gift card from payload" in new Fixture {
        val response = POST(s"v1/gift-cards",
                            GiftCardCreateByCsr(balance = 555, reasonId = 1))
        response.status must ===(StatusCodes.OK)
        ...
        val manual =
          GiftCardManuals.findOneById(root.originId).run().futureValue.value
      ...
      "succeeds with valid subTypeId" in new Fixture {
        val payload =
          GiftCardCreateByCsr(balance = 25, reasonId = 1, subTypeId = Some(1))
      ...
      "fails if subtypeId is not found" in new Fixture {
        ...
        response.error must ===(
            NotFoundFailure404(GiftCardSubtype, 255).description)
      }
      ...
      "fails to create gift card with negative balance" in new Fixture {
        val response = POST(s"v1/gift-cards",
                            GiftCardCreateByCsr(balance = -555, reasonId = 1))
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(GeneralFailure(
                "Balance got -555, expected more than 0").description)
      }

      "fails to create gift card with invalid reason" in new Fixture {
        val response = POST(s"v1/gift-cards",
                            GiftCardCreateByCsr(balance = 555, reasonId = 999))
        response.status must ===(StatusCodes.BadRequest)
    ...
    "POST /v1/gift-cards (bulk)" - {
      ...
      "fails to create multiple gift cards with zero balance" in new Fixture {
        ...
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            GeneralFailure("Balance got 0, expected more than 0").description)
      }
      ...
      "fails to create multiple gift cards with negative quantity" in new Fixture {
        ...
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(GeneralFailure(
                "Quantity got -5, expected more than 0").description)
      }
      ...
      "fails to create multiple gift cards with count more than limit" in new Fixture {
        ...
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            GeneralFailure("Quantity got 25, expected 20 or less").description)
      }
    }
    ...
    "PATCH /v1/gift-cards/:code" - {
      "successfully changes state from Active to OnHold and vice-versa" in new Fixture {
        val response = PATCH(s"v1/gift-cards/${giftCard.code}",
                             GiftCardUpdateStateByCsr(state = OnHold))
        response.status must ===(StatusCodes.OK)

        val responseBack = PATCH(s"v1/gift-cards/${giftCard.code}",
                                 GiftCardUpdateStateByCsr(state = Active))
        responseBack.status must ===(StatusCodes.OK)
      }

      "returns error if no cancellation reason provided" in new Fixture {
        val response = PATCH(s"v1/gift-cards/${giftCard.code}",
                             GiftCardUpdateStateByCsr(state = Canceled))
        response.status must ===(StatusCodes.BadRequest)
      ...
      "returns error on cancellation if gift card has auths" in new Fixture {
        val response =
          PATCH(s"v1/gift-cards/${giftCard.code}",
      ...
      "successfully cancels gift card with zero balance" in new Fixture {
        ...
        // Update balance
        GiftCards
          .update(giftCard, giftCard.copy(availableBalance = 0))
          .run()
          .futureValue
    ...
    "GET /v1/gift-cards/:code/transactions" - {
      "returns the list of adjustments" in new Fixture {
        ...
        val adjustments = response
          .ignoreFailuresAndGiveMe[Seq[GiftCardAdjustmentsResponse.Root]]
        ...
        firstAdjustment.availableBalance must ===(
            giftCard.originalBalance - adjustment1.debit)
      ...
      "returns the list of adjustments with sorting and paging" in new Fixture {
        ...
        val adjustment2 = GiftCards
          .auth(giftCard, Some(payment.id), 1)
          .run()
          .futureValue
          .rightVal
        val adjustment3 = GiftCards
          .auth(giftCard, Some(payment.id), 2)
          .run()
          .futureValue
          .rightVal

        val response = GET(
            s"v1/gift-cards/${giftCard.code}/transactions?sortBy=-id&from=2&size=2")
        val adjustments = response
          .ignoreFailuresAndGiveMe[Seq[GiftCardAdjustmentsResponse.Root]]
    ...
    "PATCH /v1/gift-cards" - {
      "successfully changes statuses of multiple gift cards" in new Fixture {
        ...
        val secondUpdated =
          GiftCards.findOneById(gcSecond.id).run().futureValue
    ...
    "POST /v1/gift-cards/:code/convert/:customerId" - {
      "successfully converts GC to SC" in new Fixture {
        val response =
          POST(s"v1/gift-cards/${gcSecond.code}/convert/${customer.id}")
        response.status must ===(StatusCodes.OK)
        ...
        val redeemedGc =
          GiftCards.findByCode(gcSecond.code).one.run().futureValue.value
      ...
      "fails to convert GC to SC if open transactions are present" in new Fixture {
        val response =
          POST(s"v1/gift-cards/${giftCard.code}/convert/${customer.id}")
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(OpenTransactionsFailure.description)
      }
      ...
      "fails to convert inactive GC to SC" in new Fixture {
        GiftCards
          .findByCode(gcSecond.code)
          .map(_.state)
          .update(GiftCard.OnHold)
          .run()
          .futureValue
        val updatedGc =
          GiftCards.findByCode(gcSecond.code).one.run().futureValue

        val response =
          POST(s"v1/gift-cards/${gcSecond.code}/convert/${customer.id}")
        response.status must ===(StatusCodes.BadRequest)
        response.error must ===(
            GiftCardConvertFailure(updatedGc.value).description)
      }
    }
  }
