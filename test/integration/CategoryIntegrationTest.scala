  ...
  "Categories" - {
    ...
    "PATCH v1/categories/:context/:formId" - {
      "returns a full category" in new Fixture {
        ...
        val response = PATCH(s"v1/categories/${context.name}/${category.form.id}",
          UpdateFullCategory(UpdateCategoryForm(updatedAttributes), UpdateCategoryShadow(updatedAttributes)))

        response.status must ===(StatusCodes.OK)

        val content = response.as[FullCategoryResponse.Root]

        val expectedFormValues: List[JString] = newValue :: testAttributes.map { case (_, value) ⇒ value }
        val formValues: List[JValue] = content.form.attributes.asInstanceOf[JObject].children
        val shadowKeys: Iterable[String] = content.shadow.attributes.asInstanceOf[JObject].values.keys
    ...
    "POST v1/categories/:context" - {
      "creates a category" in {
        ...
        val response = POST(s"v1/categories/default",
          CreateFullCategory(CreateCategoryForm(updatedAttributes), CreateCategoryShadow(updatedAttributes)))
    ...
    "GET v1/categories/:formId/form" - {
      "return form" in new Fixture {
        ...
        val expectedFormValues: List[JString] = testAttributes.map { case (_, value) ⇒ value }
    ...
    "GET v1/categories/:context/:formId/baked" - {
      "returns illuminated object" in new Fixture {
        ...
        val expected: JObject = testAttributes.map { case (key, value) ⇒ key → JObject(List("v" → value)) }
    ...
    "GET v1/categories/:context/:formId/shadow" - {
      "returns shadow" in new Fixture {
        ...
        val keys = response.as[CategoryShadowResponse.Root].attributes.asInstanceOf[JObject].values.keys
    ...
    trait Fixture {
      ...
      val category = CategoryManager.createCategory(storeAdmin, CreateFullCategory(
        CreateCategoryForm(testAttributes), CreateCategoryShadow(testAttributes)), context.name).futureValue.rightVal
    }
  }
