...
class CartValidatorTest extends IntegrationTestBase {
  ...
  "CartValidator" - {
    ...
    "has warnings" - {
      "if the cart has no items" in new Fixture {
        val result =
          CartValidator(refresh(cart)).validate().run().futureValue.rightVal
    ...
    "never has warnings for sufficient funds" - {
      "if there is no grandTotal" in new Fixture {
        ...
        result.alerts mustBe 'empty
        result.warnings.value.toList mustNot contain(
            InsufficientFunds(cart.refNum))
      }
      ...
      "if a credit card is present" in new CreditCartFixture
      with LineItemsFixture {
        val result =
          CartValidator(refresh(cart)).validate().run().futureValue.rightVal
    ...
    "has different rules for GC/SC pre-checkout and checkout funds check" - {
      "checks available balance for gift card before checkout" in new GiftCardFixture {
        val result = CartValidator(refresh(cart))
          .validate(isCheckout = false)
          .run()
          .futureValue
          .rightVal
        result.alerts mustBe 'empty
      ...
      "checks authorized funds for gift card during checkout" in new GiftCardFixture {
        ...
        val result1 = CartValidator(refresh(cart))
          .validate(isCheckout = true)
          .run()
          .futureValue
          .rightVal
        result1.alerts mustBe 'empty
        result1.warnings.value.toList must contain(
            InsufficientFunds(cart.refNum))
        // Approves authrorized funds
        GiftCards
          .auth(giftCard, orderPayment.id.some, grandTotal, 0)
          .run()
          .futureValue
          .rightVal
        val result2 = CartValidator(refresh(cart))
          .validate(isCheckout = true)
          .run()
          .futureValue
          .rightVal
        result2.alerts mustBe 'empty
        result2.warnings.value.toList mustNot contain(
            InsufficientFunds(cart.refNum))
      }
      ...
      "checks authorized funds for store credit during checkout" in new StoreCreditFixture {
        ...
        // Approves authrorized funds
        StoreCredits
          .auth(storeCredit, orderPayment.id.some, grandTotal)
          .run()
          .futureValue
          .rightVal
        val result2 = CartValidator(refresh(cart))
  ...
  trait LineItemsFixture extends Fixture {
    val (product, productForm, productShadow, sku, skuForm, skuShadow, items) =
      (for {
        context ← * <~ ObjectContexts.mustFindById404(SimpleContext.id)
        productData ← * <~ Mvp.insertProduct(context.id,
        ...
        (product, productForm, productShadow, sku, skuForm, skuShadow, items))
        .runTxn()
        .futureValue
        .rightVal
  ...
  trait LineItemsFixture0 extends Fixture {
    val (product, productForm, productShadow, sku, skuForm, skuShadow, items) =
      (for {
        context ← * <~ ObjectContexts.mustFindById404(SimpleContext.id)
        productData ← * <~ Mvp.insertProduct(
                         context.id, Factories.products.head.copy(price = 0))
        product ← * <~ Products.mustFindById404(productData.productId)
        productForm ← * <~ ObjectForms.mustFindById404(product.formId)
        productShadow ← * <~ ObjectShadows.mustFindById404(product.shadowId)
        sku ← * <~ Skus.mustFindById404(productData.skuId)
  ...
  trait CreditCartFixture extends Fixture {
    val (customer, cc) = (for {
      ...
      _ ← * <~ OrderPayments.create(Factories.orderPayment.copy(
                 orderId = cart.id, paymentMethodId = cc.id))
