...
class ShippingManagerTest extends IntegrationTestBase {
  ...
  "ShippingManager" - {
    ...
    "Evaluates rule: shipped to CA, OR, or WA" - {
      ...
      "Is true when the order is shipped to WA" in new WashingtonOrderFixture {
        val matchingMethods =
          getShippingMethodsForOrder(order.refNum).futureValue
        rightValue(matchingMethods).head.name must ===(
            shippingMethod.adminDisplayName)
      }
    ...
    "Evaluates rule: shipped to Canada" - {
      "Is true when the order is shipped to Canada" in new CountryFixture {
        ...
        OrderShippingAddresses
          .filter(_.id === orderShippingAddress.id)
          .delete
          .run()
          .futureValue
        OrderShippingAddresses
        ...
        val matchingMethods =
          getShippingMethodsForOrder(order.refNum).futureValue.rightVal
        matchingMethods.headOption.value.name must ===(
            shippingMethod.adminDisplayName)
      }
    ...
    "Evaluates rule: order total is greater than $25" - {
      ...
      "Is true when the order total is greater than $25" in new PriceConditionFixture {
        val matchingMethods =
          getShippingMethodsForOrder(expensiveOrder.refNum).futureValue
        rightValue(matchingMethods).head.name must ===(
            shippingMethod.adminDisplayName)
      }
      ...
      "Is false when the order total is less than $25" in new PriceConditionFixture {
        val matchingMethods =
          getShippingMethodsForOrder(cheapOrder.refNum).futureValue
        rightValue(matchingMethods) mustBe 'empty
      }
    }
    ...
    "Evaluates rule: order total is between $10 and $100, and is shipped to WA, CA, or OR" - {
      ...
      "Is true when the order total is $27 and shipped to CA" in new StateAndPriceCondition {
        val (address, orderShippingAddress) = (for {
          address ← * <~ Addresses.create(Factories.address.copy(
                           customerId = customer.id, regionId = washingtonId))
      ...
      "Is false when the order total is $27 and shipped to MI" in new StateAndPriceCondition {
        val (address, orderShippingAddress) = (for {
          address ← * <~ Addresses.create(Factories.address.copy(
                           customerId = customer.id, regionId = michiganId))
          orderShippingAddress ← * <~ OrderShippingAddresses.copyFromAddress(
                                    address = address, orderId = order.id)
        } yield (address, orderShippingAddress)).runTxn().futureValue.rightVal
  ...
  trait PriceConditionFixture extends Fixture {
    ...
    val (shippingMethod, cheapOrder, expensiveOrder) = (for {
      ...
      shippingMethod ← * <~ ShippingMethods.create(
                          Factories.shippingMethods.head.copy(conditions =
                                Some(conditions)))
      ...
      cheapProduct ← * <~ Mvp.insertProduct(
                        ...
                        Factories.products.head.copy(title = "Cheap Donkey",
                                                     price = 10))
      cheapLineItemSku ← * <~ OrderLineItemSkus
                          .safeFindBySkuId(cheapProduct.skuId)
                          .toXor
      ...
      expensiveProduct ← * <~ Mvp.insertProduct(productContext.id,
                                                Factories.products.head.copy(
                                                    title = "Expensive Donkey",
                                                    price = 100))
      expensiveLineItemSku ← * <~ OrderLineItemSkus
                              .safeFindBySkuId(expensiveProduct.skuId)
                              .toXor
  ...
  trait CountryFixture extends OrderFixture with ShipmentSeeds {
    ...
    val action = ShippingMethods.create(
        shippingMethods.headOption.value.copy(conditions = Some(conditions)))
    val shippingMethod = action.run().futureValue.rightVal
  }
}
