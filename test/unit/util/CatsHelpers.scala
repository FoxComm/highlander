...
trait CatsHelpers {
  this: Suite ⇒ /** For fail */
  def rightValue[A, B](
      xor: A Xor B)(implicit ttA: TypeTag[A], ttB: TypeTag[B]): B =
    xor.fold(
        l ⇒
          fail(
              s"Expected Right[${ttB.tpe.dealias}], got Left[${ttA.tpe.dealias}]: $l"),
        r ⇒ r
    )

  def leftValue[A, B](
      xor: A Xor B)(implicit ttA: TypeTag[A], ttB: TypeTag[B]): A =
    xor.fold(
        l ⇒ l,
        r ⇒
          fail(
              s"Expected Left[${ttA.tpe.dealias}], got Right[${ttB.tpe.dealias}]: $r")
    )
  ...
  def invalidValue[E, A](validated: Validated[E, A])(
    ...
    validated.fold(
        e ⇒ e,
        a ⇒
          fail(s"Expected Invalid[${ttA.tpe.dealias}], got Valid[${ttE.tpe.dealias}]: $a")
    )
