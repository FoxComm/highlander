...
begin
  ...
  update carts_search_view set
    ...
    from (select
        ...
        from carts as c
        left join order_payments as op_cc on (c.reference_number = op_cc.cord_ref and op_cc.payment_method_type = 'creditCard')
...
create trigger update_carts_view_from_billing_addresses_order_payments
    after update or insert on order_payments
    for each row
    when (new.payment_method_type = 'creditCard')
    execute procedure update_carts_view_from_billing_addresses_fn();

create trigger update_carts_view_from_billing_addresses_credit_cards
    after update or insert on credit_cards
    for each row
    execute procedure update_carts_view_from_billing_addresses_fn();

create trigger update_carts_view_from_billing_addresses_regions
    after update or insert on regions
    for each row
    execute procedure update_carts_view_from_billing_addresses_fn();

create trigger update_carts_view_from_billing_addresses_countries
