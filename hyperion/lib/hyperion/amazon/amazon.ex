defmodule Hyperion.Amazon do
  ...
  def get_full_order(order_id, token) do
    try do
      ...
      build_order_map(order, items["OrderItems"]["OrderItem"], token)
    rescue
      e ->
  ...
  def get_order_details(order_id, cfg) do
    case MWSClient.get_order([order_id], cfg) do
      {:error, error} ->
        raise %AmazonError{message: "Error while fetching order from amazon: #{inspect(error)}"}

      {:warn, warn} ->
        raise %AmazonError{message: warn["ErrorResponse"]["Error"]["Message"]}

      {_, resp} ->
        resp["GetOrderResponse"]["GetOrderResult"]["Orders"]["Order"]
    end
  end
  ...
  def get_order_items(order_id, cfg) do
    case MWSClient.list_order_items(order_id, cfg) do
      {:error, error} ->
        raise %AmazonError{
          message: "Error while fetching order details from amazon: #{inspect(error)}"
        }
      ...
      {_, resp} ->
        resp["ListOrderItemsResponse"]["ListOrderItemsResult"]
    end
  end
  ...
  defp get_sku_map(items, token) when is_list(items) do
    Enum.map(items, fn item ->
      %{
        ...
        totalPrice:
          ((String.to_float(item["ItemPrice"]["Amount"]) * 100) |> round) *
            String.to_integer(item["QuantityOrdered"]),
        productFormId: nil,
  ...
  defp get_sku_map(items, token) when is_map(items) do
    [
      %{
        ...
        totalPrice:
          ((String.to_float(items["ItemPrice"]["Amount"]) * 100) |> round) *
            String.to_integer(items["QuantityOrdered"]),
        productFormId: nil,
        trackInventory: false,
        state: "pending"
      }
  ...
  defp process_images(response) do
    ...
    case albums do
      ...
      _ ->
        Enum.map(r["skus"], fn x -> [albums: hd(albums), code: x["attributes"]["code"]["v"]] end)
    end
  ...
  def render_swatch_section(list, sku, initial) do
    ...
    |> Enum.map(fn {image, idx} ->
      [
        sku: sku,
        type: "Swatch",
        location: String.replace(image["src"], "https", "http"),
        idx: idx
      ]
    end)
  end
  ...
  defp process_products(response, set_parent \\ false) do
    ...
    products =
      Enum.map(skus, fn s -> Enum.into(s, raw_products) end)
