defmodule Hyperion.Amazon.Workers.PushCheckerWorker do
  ...
  defp get_uncomplete_pushes do
    from(
      r in SubmissionResult,
      where:
        r.completed == false and not is_nil(r.product_feed) and not is_nil(r.price_feed) and
          not is_nil(r.inventory_feed) and not is_nil(r.variations_feed) and
          not is_nil(r.images_feed)
    )
    |> Hyperion.Repo.all()
  end
  ...
  defp check_feed(push, feed_name, cfg) do
    case MWSClient.get_feed_submission_result(Map.get(push, feed_name)["FeedSubmissionId"], cfg) do
      {:error, error} ->
        Logger.error("Push ID: #{push.id} checking error: #{inspect(error)}")
  ...
  defp store_result(push_id, feed_result_name, result) do
    ...
    case Hyperion.Repo.update(row) do
      ...
      {:error, changeset} ->
        Logger.error("#{push_id}: Error while updating push with results: #{inspect(changeset)}")
    end
  end
