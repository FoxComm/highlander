...
type IStockItemUnitRepository interface {
	...
	GetUnitForLineItem(refNum string, skuCode string) (*models.StockItemUnit, error)
...
func (repository *stockItemUnitRepository) GetUnitForLineItem(refNum string, skuCode string) (*models.StockItemUnit, error) {
	unit := new(models.StockItemUnit)
	err := repository.db.
		Joins("JOIN stock_items ON stock_items.id = stock_item_units.stock_item_id").
		Where("stock_items.sku_code = ?", skuCode).
		Where("stock_item_units.ref_num = ?", refNum).
