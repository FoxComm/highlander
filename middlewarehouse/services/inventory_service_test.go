...
func (suite *InventoryServiceTestSuite) Test_CreateStockItem() {
	...
	resp, exception := suite.service.CreateStockItem(stockItem)
...
func (suite *InventoryServiceTestSuite) Test_GetStockItemById() {
	stockItem, exception := suite.service.CreateStockItem(fixtures.GetStockItem(suite.sl.ID, "SKU"))

	resp, exception := suite.service.GetStockItemById(stockItem.ID)
...
func (suite *InventoryServiceTestSuite) Test_CreateExistingStockItem() {
	...
	respUpdate, exception := suite.service.CreateStockItem(stockItemUpdate)

	suite.Nil(exception)
	suite.Equal(resp.ID, respUpdate.ID)
	suite.Equal(resp.SKU, respUpdate.SKU)

	itemUpdate, exception := suite.service.GetStockItemById(stockItem.ID)
...
func (suite *InventoryServiceTestSuite) Test_IncrementStockItemUnits() {
	stockItem, exception := suite.service.CreateStockItem(fixtures.GetStockItem(suite.sl.ID, "TEST-INCREMENT"))
	suite.Nil(exception)

	units := fixtures.GetStockItemUnits(stockItem, 1)

	exception = suite.service.IncrementStockItemUnits(stockItem.ID, models.Sellable, units)
...
func (suite *InventoryServiceTestSuite) Test_DecrementStockItemUnits() {
	unitsCount := 10
	stockItem, exception := suite.service.CreateStockItem(fixtures.GetStockItem(suite.sl.ID, "TEST-DECREMENT"))
	suite.Nil(exception)
...
func (suite *InventoryServiceTestSuite) Test_ReserveItems_SingleSKU() {
	...
	exception = suite.service.HoldItems(refNum, skus)
...
func (suite *InventoryServiceTestSuite) Test_ReleaseItems_NoReservedSKUs() {
	_, exception := suite.service.CreateStockItem(fixtures.GetStockItem(suite.sl.ID, "SKU"))
	suite.Nil(exception)

	exception = suite.service.ReleaseItems("BR10001")
	suite.NotNil(exception, "Should not be able to unreserve items while there are no reservations")
}
...
func (suite *InventoryServiceTestSuite) Test_ReleaseItems_Single() {
	...
	exception = suite.service.ReleaseItems(refNum)
	suite.Nil(exception, "Reservation should be successfully removed")
